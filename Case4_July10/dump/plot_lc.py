...
data1=(np.loadtxt(f'NB03_c4_lc_data.csv',delimiter=',',dtype='str')).transpose() #'NB03_Light_curve_data.dat'
data2=(np.loadtxt(f'NB08_c4_lc_data.csv',delimiter=',',dtype='str')).transpose() #'NB08_Light_curve_data.dat'
data3=(np.loadtxt(f'NB04_c4_lc_data.csv',delimiter=',',dtype='str')).transpose() #'NB04_Light_curve_data.dat'
data4=(np.loadtxt(f'NB02_c4_lc_data.csv',delimiter=',',dtype='str')).transpose() #'NB02_Light_curve_data.dat'
data5=(np.loadtxt(f'NB05_c4_lc_data.csv',delimiter=',',dtype='str')).transpose() #'NB05_Light_curve_data.dat'
...
float_array1_ = [float(string)  for string in data1[2]]
...
img_nm='nb3_lc.png'
axs.set_ylabel('AR region counts',fontsize=13)
...
fig,axs=plt.subplots(1,1, figsize=(10,5))
axs1=axs.twinx()
float_array3 =np.array([float(string)  for string in data3[1]])
float_array3_ = [float(string)  for string in data3[2]]
area= int(data3[3,1])
float_array_er3_=np.sqrt((float_array3/area)/area)
axs.errorbar(time_array3,float_array3/area,yerr=float_array_er3_,fmt='co-',capsize=2,markersize=2,linewidth=0.5, label='Mg II h light curve')
axs1.plot(time_array3,list(map(int,float_array3_)),color='tab:blue', marker="o",markersize=2,linewidth=0.5, label='QS light curve') 
img_nm='nb4_lc.png'
axs.set_ylabel('AR region mean counts',fontsize=13)
axs1.set_ylabel('QS region mean counts',fontsize=13)
#plt.ylabel('Total counts',fontsize=13)
plt.xlabel(f"Time ",fontsize=13)
plt.axvline(m_cls,color='orange',linestyle='--',label='GOES Flare start time')
plt.axvline(m_cls_p,color='orange',linestyle='-',label='GOES Flare peak time')
...
Helios=(np.load("cdte_data_flare_4.npy", allow_pickle=True)).transpose()
print(Helios.shape)
cdte1=Helios[1]
cdte2=Helios[2]
