...
def draw_suit_contours_on_sdo(suitMap,base_map,thresh_sig,fltrl):
    ...
    #bl = sunpy_map.pixel_to_world(bottom_left * u.pixel, y1 * u.pixel)
    # submap = suitMap.submap(bottom_left=bottom_left, top_right=top_right)
    # blo = SkyCoord(submap.bottom_left_coord.Tx, submap.bottom_left_coord.Ty, frame=submap.coordinate_frame)
    # tro = SkyCoord(submap.top_right_coord.Tx, submap.top_right_coord.Ty, frame=submap.coordinate_frame)
    # print(submap.bottom_left_coord,submap.top_right_coord)
...
ty_array=[]
pos=[]
sel_frams=[4,4,13,22,33,33]
for i in sel_frams:
    suit_map=suit_sq[i]
    pos.append(draw_suit_contours_on_sdo(suit_map,base_map,thresh_sig,fltr))
#    xt,yt=draw_suit_contours_on_sdo(suit_map,base_map,thresh_sig,fltr)
#     tx_array.append(xt)
#     ty_array.append(yt)
# tx_array=np.array(tx_array)
# ty_array=np.array(ty_array)
# print(tx_array,ty_array)
print(pos)
