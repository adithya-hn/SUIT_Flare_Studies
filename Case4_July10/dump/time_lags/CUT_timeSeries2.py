...
# === USER INPUTS ===
input_file  = "c4_diff_img_data_NB04.csv"  # your input file
...
# Slice the DataFrame
segment = df[(df['Date'] >= start_dt) & (df['Date'] <= end_dt)].copy()
segment["Time_sec"] = (segment["Date"].astype("int64") / 1e9)# seconds since 1970-01-01 , devided by 1e9 to scale down
#segment["Diff_error"]= np.sqrt(segment["diff_count"]*segment["Exposure"])/segment["Exposure"]
segment_out=segment[['Time_sec','diff_count','Diff_error']]
