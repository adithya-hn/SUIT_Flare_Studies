...
#goes=(np.loadtxt(f'/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case3_June02/onset/goes_temp_em.csv',delimiter=',',skiprows=1,dtype='str')).transpose()
goes=np.genfromtxt(f'goes_temp_em.csv', delimiter=',',names=True, dtype=None, encoding='utf-8')
time=np.array(goes['time'],dtype='datetime64')
T=np.array(goes['temperature'])
EM=np.array(goes['emission_measure'])
sT=np.ones(len(time))
sEM=np.ones(len(time))
...
# slt=Solexs[0]
# time=[datetime.strptime(str(ts)[:19], "%Y-%m-%dT%H:%M:%S") for ts in slt]
