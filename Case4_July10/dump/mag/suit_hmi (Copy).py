...
hmi_files = sorted(glob(f"/media/adithya/Adi_disk4/SUIT_flare_work/case4_jul10/data/HMI/HMI_cutouts/*.fits")) 
suit_files=sorted(glob(f'/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case4_July10/data/aligned_crop/*NB04.fits'))
...
for file in tqdm(hmi_files):
    ...
    fl_nm=jpg_fold+f'/qs_mask/'+os.path.basename(suit_files[idx])[:-4]+'jpg'
    fig=plt.figure()
    ...
    plt.close()
np.savetxt('QS_data.csv',np.c_[Date_time,qs_count,ar_1_mode,ar_1_count,ar1_sigma,ar_mode,ar_count,ar_sigma],fmt='%s',delimiter=',',comments='',header='Date,QS_mode,AR_mode,AR mean,Ar_sigma,ARmode_200G,ARmean200G,AR_sigma200G')
