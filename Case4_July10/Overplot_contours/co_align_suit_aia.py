...
Author: adithya-hn
Purpose: Overplot contours of SUIT Mg II k and Ca II H on AIA 1600 images and align SUIT images to AIA
...
def draw_suit_contours_on_aia(Map_sq,caMap_sq,jpg_fold,fltr2,files,b_files,ref_baseMap,base_time_array,ca_map_time_array,th_lvs,th_lvs2):
    ...
    for i in tqdm (range(len(files))):
        ...
        norm_mg_Map.draw_contours(axes=ax, levels=th_lvs,lws=0.5,colors=['red','pink','green'])
        norm_ca_Map.draw_contours(axes=ax, levels=th_lvs2,lws=0.5,colors=['blue','skyblue','yellow'],alpha=0.7)
...
def co_align_maps(suit_raw_files,jpg_fold,files,files2,ref_fd_1600_pth,tx1,tx2,ty1,ty2,fltr2,b_files,base_time_array,save_pngs=None,save_fits='yes',draw_contours='yes'):
    ...
    mode_val,th_lvs=get_mg_threshold(ref_mg_rot)# Mg II h/k   
    samp_ca_map=Map(files2[0])
    samp_ca_map_rot=samp_ca_map.rotate(angle=samp_ca_map.meta['CROTA2']*u.deg,missing=0)
    ca_mode_val,th_lvs2=get_ca_threshold(samp_ca_map_rot)  # Ca II H   

    print('Mode value: ', mode_val)
    print('Mode value Ca II: ', ca_mode_val)  
    print('Mg II k threshold levels:', th_lvs.astype(int)) 
    print('Ca II k threshold levels:', th_lvs2.astype(int))       

    #-------------------------------------                                                                                                                                                                                                                                                                                                      

    #--AIA template--
    ref_baseMap=get_rebinned_crop_map(ref_fd_1600,ref_mg_map)
    ...
    print('Reference image wavelength:',mg_map_sq[0].meta["WAVELNTH"])
    ...
    for i in range(len(ca_aln_maps)):
        ...
        if save_fits=='yes':
            try:
                ...
                print('No Ca II file name found, could not save file')
        if save_pngs =='yes':
            try:
    ...
    if draw_contours == 'yes':
        draw_suit_contours_on_aia(Map_sq,caMap_sq,jpg_fold,fltr2,files,b_files,ref_baseMap,base_time_array,ca_map_time_array,th_lvs,th_lvs2)



def start_coaligning(Filters,suit_raw_files,aia_imgs_pth,hmi_imgs_pth,ref_fd_1600_pth,tx1,tx2,ty1,ty2,alin_fltr,fol_nm,draw_contours='yes',save_aligned_fits='no',save_pngs='no'):
    ...
    for fltr2 in Filters:
        ...
        print(f'Searching images in {search_fold} folder')
        ...
        for f in range(len(b_files)):
            if fltr2=='1600':
                ...
                print('We currently use only AIA 1600 image for alignment, if not check the file date time format')
                print('Stopping process ...')
                break
        base_time_array=Time(parse_time(base_time_array))
        # Algining
        co_align_maps(suit_raw_files,jpg_fold,files,files2,ref_fd_1600_pth,tx1,tx2,ty1,ty2,fltr2,b_files,base_time_array,save_pngs=None,save_fits='yes',draw_contours='yes' )
...
#-----------------Intial paths and params--------------------------
    ...
    start_coaligning(Filters,suit_raw_files,aia_imgs_pth,hmi_imgs_pth,ref_fd_1600_pth,tx1,tx2,ty1,ty2,alin_fltr,fol_nm,draw_contours,save_aligned_fits,save_pngs)
