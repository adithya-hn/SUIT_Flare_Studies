...
data2=(np.loadtxt('suit_reg_lc.csv',delimiter=',',skiprows=1,dtype='str')).transpose()
#suit=(np.loadtxt('Diff_img_data_NB04.csv',delimiter=',',skiprows=1,dtype='str')).transpose()
suit_dt=np.array(data2[0],dtype='datetime64')#np.array(suit[0],dtype='datetime64')
...
suit_data=np.array(data2[1:,:], dtype=float)
print(data.shape)
...
for i in range(data.shape[0]):
    ...
    lc=np.array(data[i])
    lc2=np.array(suit_data[i])
    
    ax.plot(hmi_dt,lc,color=sns_cl3[i],label=f'Box {i}')# 
    ax2.plot(suit_dt,lc2,color='k',markersize=1.5)
