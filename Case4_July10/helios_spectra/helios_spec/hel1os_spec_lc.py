...
start_dt,end_dt=datetime.datetime(2024,7,10,3,59),datetime.datetime(2024,7,10,6,14)
...
lc_gt12 =np.sum((cdte1_counts[1:, idx_gt12]+cdte2_counts[1:, idx_gt12]), axis=1)
...
df = pd.DataFrame({'counts': lc_gt12.astype(float)}, index=pd.to_datetime(times_utc[1:]))
...
plt.yscale('log')
plt.savefig('Helios_lc_from_spec_22_kev.png',dpi=300)
plt.show()
