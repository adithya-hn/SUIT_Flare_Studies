...
data=np.loadtxt('binned_counts_10_30kev_lc.csv',delimiter=',',skiprows=1,dtype='str')
dt=np.array(data[:,0],dtype='datetime64')
counts=np.array(data[:,1],dtype=float)
# c is 1D array of counts
window = 10  # choose based on timescale over which you expect NO flares 15
...
counts_sig=np.sqrt(counts)

z = ((counts-counts_sig) - (bkg_safe+sigma ))   # "Gaussian-equivalent" significance per bin

sigma_thresh = 1.0  # or 4, 5, etc.
min_separation_bins = 2  # minimum distance between distinct peaks

peaks, props = find_peaks(
    z,
    height=0.1,    # only bins with z >= sigma_thresh
