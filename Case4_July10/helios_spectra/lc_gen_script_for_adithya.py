...
for b in range(len(all_band)):
    cdte1_lc[:,b] = np.sum(cdte1_spgm[:,all_band[b][0]:all_band[b][-1]],axis=1);
...
arf_cdte = pd.read_csv('./final_effective_area_cdte_arf.txt',header=None,delim_whitespace=True);
...
for k in range(len(cdte1_spgm)):
    ...
    # arf_ene_vec = np.mean([arf_cdte.ENERG_LO,arf_sdd2.ENERG_HI],axis=0);  # if the ARF is in fits format.
    ...
    cdte1_power = (cdte1_corr_rate*cdte1_bin_centres_inJ)/(int_arf_cdte1); # into J/s.sqm
