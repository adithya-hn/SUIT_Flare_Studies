...
files = sorted(glob.glob("/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case4_July10/data/1600_aligned/*NB02.fits"))
...
for j in range(len(aligned_maps)):  #account for incerted image, j=0 is refference image
    ...
    min_pos=np.where(aligned_img.data==np.min(aligned_img.data))
    min_coord=aligned_img.pixel_to_world(min_pos[1]*u.pixel,min_pos[0]*u.pixel)
    
    coord = SkyCoord(-200 * u.arcsec, -200*u.arcsec, frame=aligned_img.coordinate_frame)
    plt.figtext(0.04, 0.04,aligned_img.date, color='white',weight='bold', fontsize=18)
    # ax.plot_coord(min_coord, 'o')
    aligned_img.draw_contours(axes=ax,levels=np.min(aligned_img.data)*1.5,colors='white',linewidths=0.5 )
    plt.draw()
    #plt.axis('off')
    plt.tight_layout()
    plt.colorbar()
