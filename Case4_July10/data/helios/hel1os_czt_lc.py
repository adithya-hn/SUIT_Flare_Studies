...
lc_filename = f'{path}lightcurve_czt2.fits' #[i for i in os.listdir() if i.endswith('.lc.gz')][0]
...
plot_range=[datetime.datetime(2024,7,10,5,30),datetime.datetime(2024,7,10,6,10)]
...
plt.errorbar(x=plot_times_UTC1[idx1], y=plot_lcurve1[idx1], yerr=np.sqrt(plot_lcurve1[idx1]), fmt='.', capsize=5, c='b', label='Light curve (20- 40 keV)')
...
#plt.errorbar(x=plot_times_UTC5[idx5], y=plot_lcurve5[idx5], yerr=np.sqrt(plot_lcurve5[idx5]), fmt='.', capsize=5, c='m', label='Light curve (18- 160 keV)',alpha=0.5)
...
plt.savefig('c4_helios_czt_lc.png',dpi=300)
plt.show()
