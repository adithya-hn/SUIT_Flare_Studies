...
# USER INPUT
# -------------------------------------------------------
fits_files = sorted(glob.glob("/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case4_July10/data/contam_correct/*.fits"))     # list of FITS
maps = [fits.open(f) for f in fits_files]
n_maps = len(maps)
...
data_min = 0.5# min(m[0].data.min() for m in maps)
data_max = 1.5 #max(m[0].data.max() for m in maps)
...
current_map = maps[index][0].data
fig,ax=plt.subplots()
#ax = fig.add_subplot(projection=maps[0])
im = ax.imshow(current_map, vmin=initial_vmin, vmax=initial_vmax)
#ax.set_title(f"Frame {index+1}/{n_maps} {current_map.date} ")

# ---------------- Sliders for vmin / vmax ----------------
ax_vmin =0.5# plt.axes([0.15, 0.10, 0.7, 0.03])
ax_vmax =1.5# plt.axes([0.15, 0.05, 0.7, 0.03])
...
def on_key(event):
   ...
   # ax = plt.subplot(projection=maps[index])          # projection here
    #im = maps[index].plot(axes=ax,  vmin=s_vmin.val, vmax=s_vmax.val)
    ax.imshow(maps[index][0].data)
