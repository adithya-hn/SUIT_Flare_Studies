...
for fltr in Filters:
    ...
    for i in range(len(files)):
        ...
        Thresh_val= np.mean(er_box1.data)*2.8
        if fltr== 'NB08':
            Thresh_val= np.mean(er_box1.data)*1.3
    ...
    ax=plt.subplot(111)
    ax1=ax.twinx()
    ax.plot(dates, tot_count,color='k',marker='o',markersize=0.5,label='Count (after threshold)')
    ax1.plot(dates, plot_data,color='b',marker='o',markersize=0.5,label='Area (after threshold)')
    plt.gcf().autofmt_xdate()
    plt.xlabel("Time")
