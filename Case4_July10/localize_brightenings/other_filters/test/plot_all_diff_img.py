...
aia131_fl=pd.read_csv('/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case4_July10/localize_brightenings/other_filters/aia131_region_lc.csv')
...
for f in files:
    ...
    df.loc[dt > 120, 'QS_total'] = None  # gap threshold = 120 sec
    
    # Plot intensity with error bar (optional)
    #plt.errorbar(df['Time'], df['diff_count'], yerr=df['Diff_error'], alpha=0.6, label=f.split('/')[-1])
    ax.plot(df['Time'], df['QS_total']/np.max(df['QS_total']), label=f.split('/')[-1])   # simple line plot
