...
nb3_qs=np.loadtxt('csv_files/QS_data.csv', delimiter=',', skiprows=1, dtype='str').transpose()
nb3_thresh=np.array(nb3_qs[3], dtype=float)+4*np.array(nb3_qs[4], dtype=float)
...
for fltr in Filters:
    ...
    for i, suit_map in enumerate(Sequence):
        ...
        plt.imshow(img, origin='lower', cmap='gray', vmin=threshold, vmax=mode_val*4)
