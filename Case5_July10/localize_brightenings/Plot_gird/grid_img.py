...
# USER INPUT
#-----------------------------------------------------------
image_folder = "NB08/"             # path to your image directory
pattern = "*.png"
n_rows, n_cols = 3, 5

# default crop (left, right, top, bottom)
default_crop = (200,220,10,125)  

# special crop rules
first_column_crop = (200,220,10,125) #(110,220,30,125)    # example: more crop on left for column-1
last_row_crop    = (200,220,10,125) #(200,220,30,70)     # example: more crop on top for last row
...
for i, (ax, img) in enumerate(zip(axes.flatten(), images), start=1):
    ax.imshow(img)
    ax.axis('off')
    ax.text(0.14, 0.91, str(i)+')', transform=ax.transAxes,
            fontsize=6, va='top', ha='left',fontfamily='Times New Roman',
            color='white') 
#, fontweight='bold'
#plt.tight_layout()
