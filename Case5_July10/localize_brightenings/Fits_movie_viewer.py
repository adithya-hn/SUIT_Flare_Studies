...
# USER INPUT
# -------------------------------------------------------
fits_files = sorted(glob.glob("/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case5_July10/localize_brightenings/Diff_fits/minimum_NB04/*.fits"))     # list of FITS
...
norm = Normalize(vmin=initial_vmin, vmax=initial_vmax)

im = current_map.plot(axes=ax, cmap='hmimag', norm=norm)
...
def on_key(event):
    ...
    norm = Normalize(vmin=s_vmin.val, vmax=s_vmax.val)
    im = maps[index].plot(axes=ax,cmap='hmimag',norm=norm)
