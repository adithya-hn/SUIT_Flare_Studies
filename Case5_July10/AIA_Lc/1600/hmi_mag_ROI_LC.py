...
search_fold='/Analysis/Projects_Data/Flare_Data/July10_Flare_Data2/AIA_Data/1600/'
...
for i in tqdm(range (len(Sequence))):
    ...
    point1 = SkyCoord(-240*u.arcsec, -50*u.arcsec, frame=hmi_map.coordinate_frame) #wide box
    diffrot_point1 = SkyCoord(RotatedSunFrame(base=point1, duration=10*60*(i+1)*u.second)) #15min 1600
    ...
    point2 = SkyCoord(100*u.arcsec, -400*u.arcsec, frame=hmi_map.coordinate_frame) #wide box
    diffrot_point2 = SkyCoord(RotatedSunFrame(base=point2, duration=10*60*(i+1)*u.second))
...
np.savetxt(f'{param}_M1.0_Light_curve_data.csv',np.c_[date_array,fltr_count],delimiter=',',fmt='%s')
'''
plot_data=np.array(plot_data)
