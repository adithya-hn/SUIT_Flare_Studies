...
files = sorted(glob("/media/adithya/Adi_Disk3/SUIT_flare_work/Jul10_2/data/aia_fd/aia.lev1_euv_12s*131.image_lev1.fits")) 
...
np.savetxt('131_lc.csv',np.c_[times,intensities],fmt='%s',header='Date,Intensity')
plt.figure(figsize=(10, 5))
plt.plot(times, intensities, marker='o',markersize=0.5)
plt.gcf().autofmt_xdate()
plt.xlabel("Time")
plt.ylabel("Total Intensity (within 95% R$_\odot$)")
plt.title("AIA 131 Å Light Curve (Disk Only)")
plt.grid(True)
plt.show()