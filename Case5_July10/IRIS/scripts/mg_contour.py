...
filter1_fold = '/Analysis/Projects_Data/Flare_Data/July10_Flare_Data2/july10_11ut_to_18ut/'
filter2_fold = '/Analysis/Projects_Data/Flare_Data/July10_Flare_Data2/july10_11ut_to_18ut/'
...
qs_coord=SkyCoord(Tx=(-10, 90) * u.arcsec, Ty=(-55, -110) * u.arcsec, frame=filter1_map.coordinate_frame)
qs_map=filter1_map.submap(qs_coord)
filter1_map.plot()
filter1_map.draw_quadrangle(qs_coord, edgecolor="red", linestyle="-", linewidth=2, label='Region of interest')
filter1_map.peek()
...
plt.imshow(msk, origin='lower', cmap='gray', vmin=0, vmax=1)  # Adjust vmin/vmax as needed
plt.colorbar()
plt.title('Binary Mask of Filter 1 (NB03)')
plt.show()
print(np.sum(msk))
'''
#breakpoint()
...
'''
plt.imshow(filter1_data, origin='lower', cmap='gray', vmin=0, vmax=14000)  # Adjust vmin/vmax as needed
plt.colorbar()
plt.plot(largest_contour[:, 1], largest_contour[:, 0], linewidth=1.5, color='blue')
plt.title('Contour of Filter 1 (NB03)')
plt.show()'''

x_pix, y_pix = largest_contour[:, 1], largest_contour[:, 0]
# Create polygon path from contour in pixel space
contour_path = Path(np.vstack([x_pix, y_pix]).T)

# Create grid of all pixel positions
ny, nx = filter1_map.data.shape
...
fig, ax = plt.subplots()
plt.imshow(mask, origin='lower', cmap='gray', vmin=0, vmax=1)  # Adjust vmin/vmax as needed
plt.colorbar()
poly = PathPatch(contour_path, facecolor='orange', edgecolor='red', alpha=0.5)
#ax.add_patch(poly)
ax.grid()
ax.axis('equal')
plt.show()
...
for filter2_file in filter2_files:
    ...
    counts_under_contours = np.sum(np.where(mask==1, filter2_data, 0))
