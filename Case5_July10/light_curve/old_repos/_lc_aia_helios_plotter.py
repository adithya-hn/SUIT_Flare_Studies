...
Helios=(np.loadtxt("csv_files/helios_CdTe_c5.csv",skiprows=1,delimiter=',',dtype='str')).transpose()
...
flux_smooth = savgol_filter(int_roi, window_length=10, polyorder=3)
int_roi_ = np.log(np.where(flux_smooth>1.03*np.median(flux_smooth),1.033*np.median(flux_smooth),flux_smooth))#np.ma.masked_array(int_roi, mask=mask)
...
peaks, _ = find_peaks(int_roi, prominence=1, width=10)#, height=1.03*np.median(int_roi))
...
spiks=np.where((int_roi-smoothed)<50000,0,(int_roi-smoothed))
