...
files = sorted(glob.glob("/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case5_July10/data/Processed/contam_corr_data/*NB02.fits"))
print('No. of files: ',len(files))

pathlib.Path('Verify_spots').mkdir(parents=True, exist_ok=True) 
tracking_mode= 'patch' #patch /'ROI
ref_roi_x, ref_roi_y = 439, 600   # example pixel location in reference ROI
rw= 50
...
for f in files:
    ...
    if tracking_mode=='patch':
        ...
        plt.imshow(patch, origin='lower', cmap='gray',vmin=25000,vmax=35000)
        plt.plot(rw,rw,'or',markersize=10,alpha=0.5)
        plt.title(str(date)[:19]+' | shift pix: ('+str(shift_y)+' '+str(shift_x)+')')
        plt.colorbar()
        plt.savefig(f'Verify_spots/Check_patch_{i}.png',dpi=200)
        plt.close()
    if tracking_mode=='ROI':
        ...
        plt.title(str(date)[:19]+' | shift pix: ('+str(shift_y)+' '+str(shift_x)+')')
        plt.savefig(f'Verify_spots/Check_on_roi{i}.png',dpi=200)
        plt.close()
