...
# Date: 2025-03-19
#----------------
# Purpose: Extract light curve of the region of interest (ROI) and the background region for the SUIT data (non aligned).
# Description:
# This code is used to extract the light curve of the region of interest (ROI) and the background region for the SUIT data.
# The light curve is extracted for the given filters and the data is saved in a csv file.
# The images of the ROI and the background region are also saved.
...
fdir='/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case5_July10/scatter_corrected/scatter_verify/Second_sample/test3/data/raw/'
cTx1=-200
cTy1=-350
cTx2=50
cTy2=-100
...
for fltr in Filters:
    ...
    for i in range(len(Sequence)):
        ...
        fltr_count_err.append(np.median(er_box.data*1000/Sequence[i].meta.get('CMD_EXPT')))
    ...
    np.savetxt(f'{fltr}_c5_lc_data.csv',np.c_[date_array,fltr_count,fltr_count_err,bx_area],delimiter=',',fmt='%s')
