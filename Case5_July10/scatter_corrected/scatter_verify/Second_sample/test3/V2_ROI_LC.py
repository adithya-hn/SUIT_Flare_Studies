...
#Mean and median light curve for the region of interest
#--------------------------
...
fdir='/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case5_July10/scatter_corrected/scatter_verify/Second_sample/test3/data/aligned/'
...
for fltr in Filters:
    ...
    for i in range(len(Sequence)):
        ...
        coords = SkyCoord(Tx=(-200, 50) * u.arcsec, Ty=(-350, -100) * u.arcsec, frame=suit_map.coordinate_frame)
        ...
        er_coords = SkyCoord(Tx=(-10, 90) * u.arcsec, Ty=(-20, -120) * u.arcsec, frame=suit_map.coordinate_frame)
        ...
        plt.close()
        fltr_count.append(np.mean(suit_box.data*1000/Sequence[i].meta.get('CMD_EXPT')))
    ...
    np.savetxt(f'{fltr}_M1.box2_lc.csv',np.c_[date_array,fltr_count,fltr_count_err,bx_area],delimiter=',',fmt='%s')
