...
for fltr in Filters:
    ...
    for i in range(len(Sequence)):
        ...
        small_bx_count.append(np.sum(er_box.data*1000/Sequence[i].meta.get('MEAS_EXP')))#poisonian error
    ...
    small_bx_count=np.array(small_bx_count)
    ...
    np.savetxt(f'{fltr}_M1.0_Light_curve_data.csv',np.c_[date_array,fltr_count,fltr_count_err,small_bx_count],delimiter=',',fmt='%s')
