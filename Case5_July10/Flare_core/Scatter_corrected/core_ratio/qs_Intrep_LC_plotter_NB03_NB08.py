...
data=(np.loadtxt(f'core_nb08_lc_data.csv',delimiter=',',dtype='str')).transpose() #'NB08_Light_curve_data.dat'
date_array=data[0]
...
interp_func = interp1d(t2_stamps,NB3_data[2] , kind='linear', fill_value="extrapolate")
...
ax2.plot(time_array,list(map(int,float_array)),color=colors_list[1],marker='^',markersize=2,linewidth=0.5,label='Ca II h qs Total count')
axs.plot(time_array,(float_array/counts_2_interp),color=colors_list[2],marker='s',markersize=2,linewidth=0.5,label='Ca II h/Mg II k qs')

axs.set_ylabel("count Ratio ")
ax2.set_ylabel('Ca II h Total count ')
axs.set_xlabel('Time')
axis_title='Total count'
img_nm='qs_ratio_light_curve.png'

plt.xlabel('Time',fontsize=13)
plt.title('QS Light Curves')
