...
filter2_fold = '/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case5_July10/data/processed/Aligned_images/NB03/'
...
im=ref_mg_map.plot(axes=ax, cmap='gray', vmin=0, vmax=30000)
...
for contour in contours:
    ...
    hp=ref_mg_map.pixel_to_world(contour[:, 1]*u.pixel, contour[:, 0]*u.pixel)
    ax.plot_coord(hp,color='red', linewidth=1,label='Flare contour')
...
for filter2_file in filter2_files:
    ...
    filter2_map.draw_quadrangle(qs_coords_,axes=ax,edgecolor="blue",linestyle="-",linewidth=1,label='QS region')
