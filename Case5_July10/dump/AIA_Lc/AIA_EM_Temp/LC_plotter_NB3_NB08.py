...
data=(np.loadtxt(f'EM_total_counts.csv',delimiter=',',dtype='str')).transpose() #'NB03_Light_curve_data.dat'
date_array=data[0]

NB3_data=(np.loadtxt(f'NB08_M1.0_Light_curve_data.csv',delimiter=',',dtype='str')).transpose()
...
Solexs=np.loadtxt('fit_results_AL1_SOLEXS_20240710_SDD2_L1_2407101330_2407101600_TEMP_EM.txt',dtype='str',skiprows=1).transpose()
...
for i in range(len(date_array)):
    ...
    parsed_time = datetime.strptime(date_array[i], "%Y-%m-%d %H:%M:%S.%f")
...
#ax5.errorbar(sl_time,sl_Em,yerr=sl_Em_er, fmt='purple',capsize=2,markersize=2,linewidth=0.5,label="EM-SoLExs",alpha=0.5)
ax5.plot(sl_time,sl_Em,'skyblue',linewidth=0.8,label="EM-SoLExs",)
...
ax2.errorbar(nb3_time_array,list(map(int,nb3float_array)),yerr=nb3float_array_er,fmt='ko--',capsize=2,markersize=2,linewidth=0.5,label='Ca-II h')
axs.plot(time_array,float_array,'brown',linestyle='--',markersize=1,label='AIA-EM')
ax2.set_ylabel("Ca II h Total count ")
axs.set_ylabel('AIA -EM Total count')
axs.set_xlabel('Time')
