...
data=(np.loadtxt(f'{Flt1}_M1.0_Light_curve_data.csv',delimiter=',',dtype='str')).transpose() #'NB03_Light_curve_data.dat'
date_array=data[0]

NB3_data=(np.loadtxt(f'{Flt2}_M1.0_Light_curve_data.csv',delimiter=',',dtype='str')).transpose()
...
axs.errorbar(time_array,list(map(int,float_array)),yerr=float_array_er,fmt='ko-',capsize=2,markersize=2,linewidth=0.5,label=Flt1)
ax2.errorbar(nb3_time_array,list(map(int,nb3float_array)),yerr=nb3float_array_er,fmt='bo-',capsize=2,markersize=2,linewidth=0.5,label=Flt2)
#ax2.plot(nb3_time_array,hmi_data,'bo--',markersize=2,linewidth=0.5)
ax2.set_ylabel(f"{Flt2} Total count ")
axs.set_ylabel(f'{Flt1} Total count ')
ax2.set_yscale('log')
axs.legend(loc='best')
#ax2.legend(loc='best')
ax2.legend()
