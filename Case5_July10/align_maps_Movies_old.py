...
nb3_files = sorted(glob.glob(search_fold + '*3NB03.fits'))
...
for fltr in Filters:
    ...
    if fltr == 'NB03':
        ...
        for i in range (len(files)-1):
            s_maps.append(sunpy.map.Map(files[i+1]))
    ...
    for j in range(len(aligned_maps)-1):  #account for incerted image, j=0 is refference image
        aligned_img=sunpy.map.Map(aligned_maps[j+1].data,aligned_maps[j+1].fits_header)
