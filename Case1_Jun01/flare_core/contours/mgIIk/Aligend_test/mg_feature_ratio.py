...
flare_files = '/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case1_Jun01/data/processed/aligned_fits'
...
plt.savefig('Ref_map.png',dpi=300)
plt.close()
'''
...
for filter1_file in filter1_files:
    ...
    ca_rect=SkyCoord(Tx=(rct_Tx1,rct_Tx2) * u.arcsec, Ty=(rct_Ty1,rct_Ty2) * u.arcsec, frame=filter1_map.coordinate_frame)
    ...
    ca_region=filter1_map.submap(ca_rect)
    ...
    Norm_ca_map.meta.update(get_observer_meta(filter2_pos, rsun=filter2_pos.rsun))
    ...
    #print(np.median(qs_data2), np.mean(qs_data2), np.std(qs_data1))
    #print(np.median(qs_data3), np.mean(qs_data1), np.std(qs_data1))
    qs_thresh=3520#(np.median(qs_data1))#+np.median(qs_data2)+np.median(qs_data3))/3
    print('QS Threshold: ',qs_thresh+3*np.std(qs_data1) ,qs_thresh*1.5)

    plg_msk_b = (Norm_map.data > qs_thresh*1.5) & (Norm_map.data < qs_thresh*3) # True where pixel value > threshold
    kernel = disk(3)
    ...
    flare_msk_b=Norm_map.data > qs_thresh*4.5
    ...
    ax=plt.subplot(121, projection=Norm_map)
    ax2=plt.subplot(122, projection=Norm_ca_map)
    Norm_map.plot(axes=ax,autoalign=True,vmin=1000,vmax=16000)
    Norm_ca_map.plot(axes=ax2,autoalign=True,vmin=500,vmax=6500)
    th_lv=[qs_thresh*1.5,(qs_thresh)*4.5]
    for pc in range(len(plg_cont)):
        plg_hpc=Norm_map.pixel_to_world(plg_cont[pc][:, 1]*u.pixel, plg_cont[pc][:, 0]*u.pixel)
        plg_ca_hpc=plg_hpc.transform_to(Norm_ca_map.coordinate_frame)
        ax.plot_coord(plg_hpc,color='yellow')
        ax2.plot_coord(plg_ca_hpc,color='yellow')

        x_pix,y_pix = Norm_ca_map.world_to_pixel( plg_ca_hpc)
    ...
    #Norm_map.draw_quadrangle(rect_coords,axes=ax,edgecolor='blue',linestyle="-",linewidth=2,label='QS region 3')
    counts_under_contours = np.sum(np.where(m_msk==1, Norm_ca_map.data, 0))
    ...
    plage_th=np.where(((Norm_map.data>qs_thresh*1.5)&(Norm_map.data<qs_thresh*3)),Norm_map.data,0)
    ...
    results.append({'filter2_file': filter2_map.date,
                     ...
                     'ca_plage':counts_under_contours, })
