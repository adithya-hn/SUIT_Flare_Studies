...
ca_plage=[float(string) for string in data[6]]
...
ca_plage_norm=ca_plage/plage_area
mg_plage_norm=plage_count/plage_area
ratio=ca_plage_norm/mg_plage_norm
...
plt.plot(time_array,plage_count/plage_area,'ko-',markersize=2,linewidth=0.5,label='Mg plage')

Flt=param
axis_title='Mg II h plage intensity'
...
plt.plot(time_array,ratio,label='Mean QS')
#plt.plot(time_array,qs_count1,label=' QS1')
#plt.plot(time_array,qs_count2,label=' QS2')
#plt.plot(time_array,qs_count3,label=' QS3')
Flt=param
axis_title='ratio'
img_nm=Flt+'_ratio_light_curve.png'

plt.ylabel(axis_title,fontsize=13)
plt.xlabel('Time',fontsize=13)
plt.axvline(m_cls,color='b',linestyle='--',label='GOES Flare start time')
plt.axvline(m_cls_p,color='b',linestyle='-',label='GOES Flare peak time')
plt.title('CaII h /Mg II k plage ratio light curve')
plt.legend(loc='best')
time_formatter = mdates.DateFormatter('%H:%M')  # Format as HH:MM
plt.gca().xaxis.set_major_formatter(time_formatter)
plt.savefig(img_nm,dpi=300)
...
plt.plot(time_array,ca_plage_norm,'ko-',markersize=2,linewidth=0.5,label='ca plage count')
xis_title='Mean count'
img_nm='NB08_ca_plage_light_curve.png'

plt.ylabel(axis_title,fontsize=13)
plt.xlabel('Time',fontsize=13)
plt.axvline(m_cls,color='b',linestyle='--',label='GOES Flare start time')
plt.axvline(m_cls_p,color='b',linestyle='-',label='GOES Flare peak time')
plt.title('CaII h light curve')
plt.legend(loc='best')
time_formatter = mdates.DateFormatter('%H:%M')  # Format as HH:MM
plt.gca().xaxis.set_major_formatter(time_formatter)
plt.savefig(img_nm,dpi=300)
