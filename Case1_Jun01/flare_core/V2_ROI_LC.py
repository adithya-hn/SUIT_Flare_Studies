...
for fltr in Filters:
    ...
    for i in range(len(Sequence)):
        ...
        coords = SkyCoord(Tx=(-570, -410) * u.arcsec, Ty=(-300, -200) * u.arcsec, frame=suit_map.coordinate_frame)
        suit_map.draw_quadrangle(coords,axes=ax,edgecolor="red",linestyle="-",linewidth=2,label='Region of interest')
        er_coords = SkyCoord(Tx=(-400, -310) * u.arcsec, Ty=(-400, -475) * u.arcsec, frame=suit_map.coordinate_frame)
    ...
    np.savetxt(f'{fltr}_X1.4_Light_curve_data.csv',np.c_[date_array,fltr_count,fltr_count_err,bx_area],delimiter=',',fmt='%s')
