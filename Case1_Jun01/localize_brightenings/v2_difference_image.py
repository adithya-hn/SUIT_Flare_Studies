...
nb3_maps=glob.glob('/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case1_Jun01/data/1600_aligned/*NB08.fits')
...
date=[]
hist_data=[]
pathlib.Path("Diff_imgs/NB03").mkdir(parents=True, exist_ok=True)
pathlib.Path("Diff_hists").mkdir(parents=True, exist_ok=True)



def cut_out_submap(smap):
    rotation_angle=smap.meta["CROTA2"]
    cen_cord      = SkyCoord(cTx1 * u.arcsec,cTy1* u.arcsec, frame=smap.coordinate_frame)
    ...
    return smap.submap(coords)

'''rotation_angle=m_seq[0].meta["CROTA2"]
cen_cord      = SkyCoord(cTx1 * u.arcsec,cTy1* u.arcsec, frame=m_seq[0].coordinate_frame)
offset_frame1 = SkyOffsetFrame(origin=cen_cord, rotation=-rotation_angle*u.deg)
width1  = arW * u.arcsec
height1 = arH * u.arcsec
coords = SkyCoord(lon=[-1/2, 1/2] * width1, lat=[-1/2, 1/2] * height1, frame=offset_frame1)
cutout=m_seq[0].submap(coords)'''

base_img_=np.zeros_like(m_seq[0].data[30:480,25:480])

for j in range(5):
    cutout=m_seq[j].data[30:480,25:480]
...
for k in range(5):
    ...
    diff_img=((m_seq[k]).data[30:480,25:480]*1000/m_seq[k].meta.get('CMD_EXPT'))-(base_map.data)
...
for i in range(len(m_seq)-1):
    fig = plt.figure()
    cutout=m_seq[i].data[30:480,25:480]
...
#plt.yscale('log')
np.savetxt('NB08_brightenings.csv',np.c_[date,count],delimiter=',',comments='',header='DateTime,Diff_count',fmt='%s')
