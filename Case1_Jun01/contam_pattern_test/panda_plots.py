...
drift=pd.read_csv('case1_crpix_values_2k.csv', parse_dates=['time'])
...
for par in params:
    ...
    ax2.plot(drift['time'],drift['crpix1'],color='tab:green',label='2K CRPIX1')
    ax2.set_ylim(1270,1290)
    plt.xlabel('Time')
    plt.ylabel('Counts')
    plt.title('NB04 Light Curve')
    ax.legend(loc='upper right')
    ax2.legend(loc='upper left')
    #plt.grid(True)
    plt.savefig(f'{par}.png',dpi=300)
    plt.close()
