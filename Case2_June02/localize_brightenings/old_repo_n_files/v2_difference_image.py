...
nb3_maps=glob.glob('/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case2_June02/data/1600_aligned/*NB08.fits')
...
for i in range(len(m_seq)-1):
    ...
    plt.savefig(f'Diff_imgs/NB03/{m_seq[i].date}.png')
    plt.close()
...
#plt.yscale('log')
np.savetxt('NB03_brightenings.csv',np.c_[date,count],delimiter=',',comments='',header='DateTime,Diff_count',fmt='%s')
plt.savefig('nb03base_simga_thresh_difference.png',dpi=300)
plt.show()
