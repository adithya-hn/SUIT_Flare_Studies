...
Mod History:
    -> Added TOTPOT, USJH, USJZ,  image products, Adithya (11-06-2024)
    -> Added Shear angle and Polirity inversion image return lines, Adithya (12-07-2204)
"""
...
def computeJzmoments(jz, jz_err, derx, dery, conf_disambig, bitmap, nx, ny, rsun_ref, rsun_obs, cdelt1_arcsec, munaught):
    ...
    err        = 0.0
    us_i       = 0.0
    USJz_Img       =np.zeros((ny,nx))
    USJz_er_Img    =np.zeros((ny,nx))
    ...
    for j in range(ny):
        for i in range(nx):
            ...
            count_mask += 1
            USJz_Img[j,i]=abs(jz[j,i])*(cdelt1_arcsec/1)*(rsun_ref/rsun_obs)*(0.00010)*(1/munaught)
            USJz_er_Img[j,i]=(jz_err[j,i]*jz_err[j,i])
    ...
    return [mean_jz, mean_jz_err, us_i, us_i_err,USJz_Img,USJz_er_Img]
...
def computeHelicity(jz, jz_err, bz, bz_err, conf_disambig, bitmap, nx, ny, rsun_ref, rsun_obs, cdelt1_arcsec):
    ...
    sum2            = 0.0
    err             = 0.0
    US_JH_img       =np.zeros((ny,nx))
    JH_err_img      =np.zeros((ny,nx))
    ...
    for j in range(ny):
        for i in range (nx):
            ...
            count_mask += 1
            US_JH_img[j,i]=abs(jz[j,i]*bz[j,i])*(1/cdelt1_arcsec)*(rsun_obs/rsun_ref) 
            JH_err_img[j,i]=(jz_err[j,i]*jz_err[j,i]*bz[j,i]*bz[j,i]) + (bz_err[j,i]*bz_err[j,i]*jz[j,i]*jz[j,i])
    ...
    return [mean_ih, mean_ih_err, total_us_ih, total_us_ih_err, total_abs_ih, total_abs_ih_err,US_JH_img,JH_err_img]
...
#===========================================
# Added 2D mtx to collect idividual pixel values
def computeFreeEnergy(bx_err, by_err, bx, by, bpx, bpy, nx, ny, rsun_ref, rsun_obs, cdelt1_arcsec, conf_disambig, bitmap):
    ...
    sum1            = 0.0
    err             = 0.0
    tot_pot_img=np.zeros((ny,nx))
    tot_pot_img_er=np.zeros((ny,nx))
    ...
    for j in range(ny):
        for i in range (nx):
            ...
            count_mask += 1
            tot_pot_img[j,i]=( ((bx[j,i] - bpx[j,i])*(bx[j,i] - bpx[j,i])) + ((by[j,i] - bpy[j,i])*(by[j,i] - bpy[j,i])) )*(cdelt1_arcsec*cdelt1_arcsec*(rsun_ref/rsun_obs)*(rsun_ref/rsun_obs)*100.0*100.0)/(8.*np.pi)
            tot_pot_img_er[j,i]=4.0*(bx[j,i] - bpx[j,i])*(bx[j,i] - bpx[j,i])*(bx_err[j,i]*bx_err[j,i]) + 4.0*(by[j,i] - bpy[j,i])*(by[j,i] - bpy[j,i])*(by_err[j,i]*by_err[j,i])/(8.*np.pi)
    ...
    return [meanpot, meanpot_err, totpot, totpot_err,tot_pot_img,tot_pot_img_er]
...
def computeShearAngle(bx_err, by_err, bz_err, bx, by, bz, bpx, bpy, nx, ny, conf_disambig, bitmap):
    ...
    shear_img=np.zeros((ny,nx))
    shear_img_er=np.zeros((ny,nx))
    ...
    return [meanshear_angle, meanshear_angle_err, area_w_shear_gt_45,shear_img,shear_img_er]
...
def computeR(los, los_err, nx, ny, cdelt1_arcsec):
    ...
    PIL_img       =np.zeros((ny,nx))
    PIL_err_img      =np.zeros((ny,nx))
    ...
    for j in range(ny1):
        for i in range (nx1):
            ...
            PIL_img[j,i]=pmap[j,i]*abs(rim[j,i])
    ...
    return [Rparam, Rparam_err,PIL_img]
