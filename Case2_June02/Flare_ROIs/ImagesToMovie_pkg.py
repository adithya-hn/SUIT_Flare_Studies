...
def Make_movie(image_folder, video_name, frame_rate):
    ...
    images=sorted(images, key=lambda file_name: datetime.datetime.strptime(os.path.basename(file_name).split('_')[5], "%Y-%m-%dT%H.%M.%S.%f"))#sort()  # Ensure the images are in order
