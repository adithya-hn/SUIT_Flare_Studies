...
Filter='NB08'
search_fold=f'/Analysis/Projects_Data/Flare_Data/June01_Flare_Data/Flare_data/' #Custom Folder
...
#ref_pth='/Analysis/Projects_Data/Flare_Data/June01_Flare_Data/Flare_data/SUT_T24_0785_000396_Lev1.0_2024-06-01T08.44.40.952_0973NB03.fits'
ref_img=sunpy.map.Map(files[0])

coords_ = SkyCoord(Tx=(-650, -270) * u.arcsec,Ty=(-100, -450) * u.arcsec,frame=ref_img.coordinate_frame,)
...
for i in range(len(files)-d):
    ...
    coords = SkyCoord(Tx=(-650, -300) * u.arcsec,Ty=(-140, -420) * u.arcsec,frame=suitMap.coordinate_frame,)
    ...
    #initial_point = SkyCoord(-230*u.arcsec, -510*u.arcsec, frame=Diff_Map.coordinate_frame)
    #point_plot = ax.scatter(initial_point.Tx.value, initial_point.Ty.value, color='white', s=90,marker='+')
    
    Diff_Map.plot(cmap=filterColor[Filter], vmin=0 ,vmax=0.7) #nb08 =0.5
    ax.plot_coord(SkyCoord(-487*u.arcsec, -207*u.arcsec, frame=Diff_Map.coordinate_frame),color='white',marker='X',markersize=12)
