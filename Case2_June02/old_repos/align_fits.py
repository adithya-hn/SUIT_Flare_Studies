...
fol_nm='/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case2_June02/data/Processed/'
...
search_fold='/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case2_June02/data/raw/' #'/scratch/suit_data/level1fits/2024/'+str(now.month).zfill(2)+'/'+str(now.day).zfill(2)+'/'+'normal_2k'+'/'
fdir =search_fold 
...
ani = aligned_maps.plot(axes=ax,cmap='gray')
#plt.axis('off')
ani.save(mv_nm)
plt.close()
...
for j in range(len(aligned_maps)):  #account for incerted image, j=0 is refference image
    ...
    aligned_img.save(algn_dir+'/'+os.path.basename(files[j]),overwrite=True) #need this for alignement refference
    aln_imgs.append(algn_dir+'/'+os.path.basename(files[j]))
    fl_nm=jpg_fold+'/'+os.path.basename(files[j])[:-4]+'jpg'
    #Title_=aligned_img.meta.get('FTR_NAME') +' Filter: '+aligned_img.date
    fig=plt.figure(figsize=(10,10))
    ax = fig.add_subplot(111, projection=aligned_img)
    aligned_img.plot(title=False,cmap='viridis',clip_interval=(2, 99.95) * u.percent)
