...
def draw_suit_contours_on_sdo(suitMap,aia_map,base_map,thresh_sig,fltr,aia_dt):

    pathlib.Path(jpg_fold+f'/{fltr}_171').mkdir(parents=True, exist_ok=True)
    ...
    fl_nm=jpg_fold+f'/{fltr}_171'+'/'+os.path.basename(suitMap.meta['F_NAME'])[:-4]+'jpg'
...
#-----------------Intial paths and params--------------------------
    ...
    suit_aligned_files= '/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case2_June02/data/aligned_crop/'
    aia_imgs='/media/adithya/Adi_disk4/SUIT_flare_work/case2_Jun02/data/aia/cut_outs/171_cutouts/'
...
#------------------------------------------------------------------
    ...
    for i in tqdm(range(len(nb4_fls))):
        ...
        idx=np.argmin(np.abs(aia_dt_array - base_time))
        aia_map=Map(aia_fls[idx])
        aia_dt=aia_map.date
        if i==0:
            ref_aia_map=Map(aia_fls[idx])
        ...
        draw_suit_contours_on_sdo(suit_map,aia_map_drot,base_map,thresh_sig,fltr,aia_dt)
