...
rotation_angle=suit_ref_map.meta["CROTA2"]
...
for file in tqdm(hmi_files):
    ...
    Date_time.append(suitMap.date)
    ar_count.append(np.sum(MgII_data*binary_ar))
    #print(qs_count)
    '''
    ...
    plt.colorbar()
    plt.savefig(f'Ar_mask_{suitMap.date}.png',dpi=300)
np.savetxt('QS_data.csv',np.c_[Date_time,qs_count,ar_count],fmt='%s',delimiter=',',comments='',header='Date,QS_mode,AR sum')
