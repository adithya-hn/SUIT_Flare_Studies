...
peak_time=datetime.datetime(2024, 11,13 ,17,8, 0)
start_time=datetime.datetime(2024, 11,13 ,16,57, 0)
suit_aligned_files= '/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case10_Nov13/data/aligned_crop/'
pk_img="/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case10_Nov13/data/aligned_crop/SUT_T24_1592_000653_Lev1.0_2024-11-13T17.05.44.809_0973NB04.fits"
base_image='SDO' #suit
if base_image=='SDO':
    aia_fl='/media/adithya/Adi_disk4/SUIT_flare_work/case10_nov13/data/HMI/HMI_cutouts/test_set/hmi.m_45s.20241113_150815_TAI.2.magnetogram.fits'#'/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case4_July10/data/aia/aia.lev1_euv_12s.2024-07-10T040010Z.171.image_lev1.fits'
    hmi_imgs   = '/media/adithya/Adi_disk4/SUIT_flare_work/case10_nov13/data/HMI/HMI_cutouts/test_set/'
fltr='NB04'
thresh_sig=5
...
if "CROTA2" in hmi_map_.meta:
    ...
    if map_rot_angl>5:
        aia_map_=hmi_map_.rotate(angle=-map_rot_angl*u.deg)
...
if base_image=='SDO':
    #im2=ax.imshow(aia_map.data/1e3,cmap=sns_cl,alpha=1,vmin=-2,vmax=2,transform=ax.get_transform(aia_map.wcs))
    im2=ax.imshow(base_map.data/1e3,cmap='suit_nb04',norm=norm)
else:
    base_map.plot(axes=ax)
peak_map.draw_contours(axes=ax,levels=[np.max(peak_map.data)*.6],colors='yellow',lw=.5)
sns_cl = sns.color_palette("coolwarm",as_cmap=True)
...
im=ax.imshow(heat_masked/1000, origin='lower', cmap='rainbow', alpha=0.45)
#ax.imshow(masked_thinned_MPIL,cmap='spring')
#mask_pil_map.plot(axes=ax,cmap='binary

#ax.imshow(masked_thinned_MPIL,transform=ax.get_transform(hmi_map_.wcs)) #bset
...
ax.set_xlim(80,600)
ax.set_ylim(80,600)
#plt.savefig(f'c9_HMI_{fltr}_stacked_contours.pdf',dpi=300)
plt.savefig(f'c10_{fltr}_stacked_contours.pdf',dpi=300)
plt.show()
