...
csv_file = f'csv_files/aia_{131}_all_lc.csv'
...
times = [datetime.strptime(d, "%Y-%m-%dT%H:%M:%S.%f") if '.' in d else datetime.strptime(d, "%Y-%m-%dT%H:%M:%S") for d in dates_str]
...
ll=np.power(10,5.4)
#axs1.set_ylim(ll,ul)
axs[0].axvspan(xmin=datetime(2024,11,13,15,35),xmax=datetime(2024,11,13,15,41),color='r',alpha=0.2)
axs[0].axvspan(xmin=datetime(2024,11,13,16,11),xmax=datetime(2024,11,13,16,24),color='r',alpha=0.2)
axs[0].axvspan(xmin=datetime(2024,11,13,15,59),xmax=datetime(2024,11,13,16,8),color='r',alpha=0.2)
...
# Add global title
fig.suptitle(f"Flare Light Curves (SOL2024-11-01T17:08) ", fontsize=20, weight='bold')
