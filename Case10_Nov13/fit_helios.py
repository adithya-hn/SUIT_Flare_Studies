...
data_dir='/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case10_Nov13/data/helios/'

lc_filename =data_dir+'lightcurve_cdte1.fits' #[i for i in os.listdir() if i.endswith('.lc.gz')][0]
gti_filename = data_dir+'gticdte1.fits'
...
idx = (plot_times_UTC>datetime.datetime(2024,11,13,0,0)) & (plot_times_UTC<datetime.datetime(2024,11,13,17,30))
plt.errorbar(x=plot_times_UTC[idx], y=plot_lcurve[idx], yerr=np.sqrt(plot_lcurve[idx]), fmt='.', capsize=5, c='b', label='Light curve');
if(gti_filename is not None):
    for n,i,j in zip(np.arange(len(gti_start_arr_UTC)), gti_start_arr_UTC, gti_stop_arr_UTC):
        plt.axvspan(i,j, color='green', alpha=0.5, label='_'*n + 'Good Time Intervals')
integration_interval = (datetime.datetime(2024,11,13,16,54), datetime.datetime(2024,11,13,17,2)) # not clear after this step
...
pi_filename = data_dir+'hel1os_cdte_spectra_cdte1.fits'
gti_filename = data_dir+'gticdte1.fits'
...
cdte_arf_filename = data_dir+'CdTeResponseReader/hel1os_cdte_arf_v03.fits'
...
cdte_srf_filename = data_dir+'CdTeResponseReader/hel1os_cdte_srf_v03.fits'
