...
@Description
-----------
Brief description: Broken power law fitted but second powerlaw not optmised, fixed the break energy
...
matched_ints = []
all_c = np.array(np.concatenate(all_ints))
all_t = np.array(np.concatenate(all_times))
np.savetxt('full_data_unsorted.csv',np.c_[all_t.astype('str'),all_c],delimiter=',',header='date_time,intensity',comments='',fmt='%s')
...
Eb1_opt=0.35 #0.24 #
Eb2_opt=1.0
print("Fixed Eb1, Eb2:", Eb1_opt, Eb2_opt)
...
# def fit_all(E, Eb1_init, Eb2_init):
#     from scipy.optimize import minimize
#     import numpy as np
...
#         L = (
#             logL(seg1, Emin, Eb1, a1) +
#             logL(seg2, Eb1, Eb2, a2) +
#             logL(seg3, Eb2, Emax, a3)
#         )
        
#         # mild regularization (prevents degeneracy)
#         penalty = 0.05*((a2 - a1)**2 + (a3 - a2)**2)
        
#         return -(L / len(E)) + penalty

#     # initial guess from BIC (VERY IMPORTANT)
#     x0 = [
#         np.log(Eb1_init),
#         np.log(Eb2_init - Eb1_init),
#         np.random.uniform(1.05,1.3),
#         np.random.uniform(1.4,1.9),
#         np.random.uniform(2.0,3.5)
#     ]

#     res = minimize(neg_logL,
#                    x0=x0,
#                    method='Powell',
#                    options={'maxiter':500})
...
#     results.append(fit_all(sample, Eb1_opt, Eb2_opt))

# results = np.array(results)

# Eb1_mean, Eb1_std = np.mean(results[:,0]), np.std(results[:,0])
# Eb2_mean, Eb2_std = np.mean(results[:,1]), np.std(results[:,1])

# a1_mean, a1_std = np.mean(results[:,2]), np.std(results[:,2])
# a2_mean, a2_std = np.mean(results[:,3]), np.std(results[:,3])
# a3_mean, a3_std = np.mean(results[:,4]), np.std(results[:,4])
...
plt.loglog(E_sorted, ccdf, '+',color='k', label='SUIT transeint events')
# Segment masks
m1 = (E_sorted < Eb1_opt)
