...
fdir='/Analysis/Projects_Data/Flare_Data/Flare_Quiet_case/June06/Processed/Aligned_images/'
...
for fltr in Filters:
    ...
    for i in range(len(Sequence)):
        ...
        coords = SkyCoord(Tx=(0, 200) * u.arcsec, Ty=(-225, -375) * u.arcsec, frame=suit_map.coordinate_frame)
        ...
        er_coords = SkyCoord(Tx=(50, 250) * u.arcsec, Ty=(-50, -200) * u.arcsec, frame=suit_map.coordinate_frame)
        ...
        fltr_count_err.append(np.sum(er_box.data*1000/Sequence[i].meta.get('MEAS_EXP')))#np.sqrt(er_area)*(np.std(er_box.data))*1000/Sequence[i].meta.get('MEAS_EXP'))
    ...
    np.savetxt(f'{fltr}_Quiet_AR_Light_curve_data.csv',np.c_[date_array,fltr_count,fltr_count_err],delimiter=',',fmt='%s')
