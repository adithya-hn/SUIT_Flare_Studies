...
for fltr in Filters:
    ...
    for i in range(len(files)):
        suitMap_=sunpy.map.Map(files[i]) #ca image
        coords = SkyCoord(Tx=(-325, -200) * u.arcsec, Ty=(-250, -170) * u.arcsec, frame=suitMap_.coordinate_frame)
        #suitMap_.draw_quadrangle(coords,axes=ax,edgecolor="red",linestyle="-",linewidth=2,label='Region of interest')
        suitMap=suitMap_.submap(coords)
        ...
        ax = fig.add_subplot(111, projection=suitMap_)
        ...
        a=normsuitMap.draw_contours(axes=ax, levels=th_lvs[0],zorder=1,fill=True,colors=['skyblue','yellow'])
        Mg_Map.draw_contours(axes=ax, levels=th_lvs2,zorder=2,colors=['pink','green'],alpha=0.5)
        
        #print(a.collections[0].get_paths()[0])
        xy_coords=a.get_paths()[0].vertices
        y = (xy_coords[:,0]).astype(int)
        x = (xy_coords[:,1]).astype(int)
        mask_img=np.zeros_like(MgII_data)
        mask_img[y,x] = 100 
        plt.imshow(mask_img)
