...
for k in range(len(sl_time)-2):
    #print(np.mean(sl_temp[k]+sl_temp[k+1]+sl_temp[k+2]))
    sm_temp.append(np.sum(sl_temp[k]+sl_temp[k+1]+sl_temp[k+2])/3)
    sm_temp_er.append(np.sqrt(pow(sl_temp_er[k],2)+pow(sl_temp_er[k+1],2)+pow(sl_temp_er[k+2],2))/3)
    sm_time.append(sl_time[k+1])
    sm_em.append(np.sum(sl_Em[k]+sl_Em[k+1]+sl_Em[k+2])/3)
    sm_em_er.append(np.sqrt(pow(sl_Em_er[k],2)+pow(sl_Em_er[k+1],2)+pow(sl_Em_er[k+2],2))/3)



#plt.errorbar(sl_time,sl_temp,yerr=sl_temp_er,alpha=0.5)
#plt.errorbar(sm_time,sm_temp,yerr=sm_temp_er, fmt='g',capsize=1.5,markersize=2,linewidth=0.5,label="Temperature-SoLExs")

#plt.errorbar(sm_time,sm_em,yerr=sm_em_er, fmt='r',capsize=1.5,markersize=2,linewidth=0.5,label="EM-SoLExs")
#plt.show()
...
#ax3.set_ylabel('Helios (Counts)',fontsize=13)
#ax3.set_yscale('log')


#ax4.spines.right.set_position(("axes", 1.33))
ax4.errorbar(sm_time,sm_temp,yerr=sm_temp_er, fmt='g',capsize=2,markersize=2,linewidth=0.5,label="Temperature-SoLExs",alpha=0.5)
ax4.set_ylabel('Temperature (MK)',fontsize=13)
ax4.set_yscale('log')#'''

#ax5.spines.right.set_position(("axes", 1.1))
ax5.errorbar(sm_time,sm_em,yerr=sm_em_er, fmt='magenta',capsize=1.5,markersize=2,linewidth=0.5,label="EM-SoLExs")
