...
for idx in gap_indices:
    axs[0].errorbar(time_array1[start:idx+1], (lc1_mean)[start:idx+1]/10e8,yerr=lc1_mean_er[start:idx+1]/10e8,fmt='tab:blue', marker="o",capsize=2,markersize=2,linewidth=0.5, label="SUIT Mg II h")
    axs1.plot(time_array5[start:idx+1],nb3_counts[start:idx+1],'y',label=r'Difference image intensity above 4 $\sigma$')
    start = idx + 1
axs[0].errorbar(time_array1[start:], (lc1_mean)[start:]/10e8,yerr=lc1_mean_er[start:]/10e8,fmt='tab:blue', marker="o",capsize=2,markersize=2,linewidth=0.5)
axs1.plot(time_array5[start:],nb3_counts[start:],'y')
