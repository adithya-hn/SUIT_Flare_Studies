...
Helios=(np.loadtxt("csv_files/helios_CdTe_c8.csv",skiprows=1,delimiter=',',dtype='str')).transpose()
...
int_roi = rgn_lc_data[:, 5].astype(float)/exposure #region 4
...
dt=np.array(times)
#------
int_roi_ = np.log(np.where(int_roi>2*np.median(int_roi),2*np.median(int_roi),int_roi))#np.ma.masked_array(int_roi, mask=mask)
from scipy.signal import savgol_filter
#int_roi_ = savgol_filter(int_roi_, window_length=5, polyorder=3)
# peaks, properties = find_peaks(int_roi,  width=5)
# #properties["prominences"], properties["widths"]

# peaks=np.array(argrelextrema(int_roi_,np.greater)[0])
# peaks_sort=np.argsort(int_roi_[peaks])[::-1]
# peaks=peaks[peaks_sort[:3]]

peaks, _ = find_peaks(int_roi, prominence=1, width=(4,7))
