...
data2=(np.loadtxt(f'csv_files/c{C_n}_NB08_lc_data.csv',delimiter=',',skiprows=1,dtype='str')).transpose() 
...
cdte1_er=np.array(Helios[1], dtype=np.float64)
cdte2_er=np.array(Helios[2], dtype=np.float64)
cdte_er=np.sqrt(cdte1_er**2+cdte2_er**2)
...
fig, axs = plt.subplots(5, 1, sharex=True, figsize=(12,17),
...
sole1=150
#axs1_=axs[1].twinx()
axs[0].errorbar(time_array1[:nb3_c1], (lc1_mean)[:nb3_c1]/10e7,yerr=lc1_mean_er[:nb3_c1]/10e7,fmt='tab:blue', marker="o",capsize=2,markersize=2,linewidth=0.5, label="SUIT Mg II h"); axs[0].legend(loc='lower right')
axs[0].errorbar(time_array1[nb3_c1:], (lc1_mean)[nb3_c1:]/10e7,yerr=lc1_mean_er[nb3_c1:]/10e7,fmt='tab:blue', marker="o",capsize=2,markersize=2,linewidth=0.5); axs[0].legend(loc='lower right')
axs[1].errorbar(time_array2[:nb8_c1], (lc2_mean)[:nb8_c1]/10e7,yerr=lc2_mean_er[:nb8_c1]/10e7,fmt='black', marker="o",capsize=2,markersize=2,linewidth=0.5, label="SUIT Ca II H"); axs[1].legend(loc='lower right')
axs[1].errorbar(time_array2[nb8_c1:nb8_c2], (lc2_mean)[nb8_c1:nb8_c2]/10e7,yerr=lc2_mean_er[nb8_c1:nb8_c2]/10e7,fmt='black', marker="o",capsize=2,markersize=2,linewidth=0.5); axs[1].legend(loc='lower right')
axs[1].errorbar(time_array2[nb8_c2:], (lc2_mean)[nb8_c2:]/10e7,yerr=lc2_mean_er[nb8_c2:]/10e7,fmt='black', marker="o",capsize=2,markersize=2,linewidth=0.5); axs[1].legend(loc='lower right')
...
axs1=axs[0].twinx()
axs2=axs[1].twinx()
axs1.plot(time_array1[:nb3_c1],nb3_counts[:nb3_c1],'y')
axs1.plot(time_array1[nb3_c1:],nb3_counts[nb3_c1:],'y')
axs2.plot(time_array2[:nb8_c1],nb8_counts[:nb8_c1],'y')
axs2.plot(time_array2[nb8_c1:nb8_c2],nb8_counts[nb8_c1:nb8_c2],'y')
axs2.plot(time_array2[nb8_c2:],nb8_counts[nb8_c2:],'y')
axs2.set_yscale('log')
axs1.set_yscale('log')


 
axs[1].set_ylabel(r'Ca II H counts ($\times 10^{7}$)', fontsize=20)
axs[0].set_ylabel(r'Mg II h counts ($\times 10^{7}$)', fontsize=20)
axs[2].set_ylabel('HEL1OS counts/min', fontsize=20)
axs[4].set_ylabel(r'EM($\times 10^{43}cm^{-3}$)', fontsize=20)
axs[3].set_ylabel('Temperature (MK)', fontsize=20)
axs2.set_ylabel(r'Mg II h enhancemnets', fontsize=20)
axs1.set_ylabel(r'Ca II H enhancemnets', fontsize=20)
axs[2].set_yscale('log')
axs[4].set_yscale('log')
axs[-1].set_xlabel(f"Start Time ({date[:19]})", fontsize=20) # Shared x-label

time_formatter = mdates.DateFormatter('%H:%M')  # Format as HH:MM
plt.gca().xaxis.set_major_formatter(time_formatter)
# Add global title
fig.suptitle(f"Flare Light Curves ", fontsize=24, weight='bold')
