...
if OPTS.noplot:
    ...
    print('Max DCF val',max(DCF),'+-',DCFERR[idx], '|','Lag in seconds: ',T[idx])
    print('Need shift light curve 1 by ' ,T[idx] ,'seconds')

    def gaus(X,C,X_mean,sigma):
        return C*np.exp(-(X-X_mean)**2/(2*sigma**2))

    mean = T[idx] #max decf position                
    sigma = OPTS.dt[0] #assuming the lag in bin width is sigma as initiall guess
    p0= [max(DCF), mean, sigma]  #initial guess for the gaussian fit
    #Gaussian least-square fitting process
    param_optimised,param_covariance_matrix = curve_fit(gaus,T,(DCF+1.5),p0=p0,sigma=DCFERR,absolute_sigma=True,maxfev=5000)
    a_fit, mu_fit, sig_fit = param_optimised
    print('lag fit',mu_fit, '±',np.sqrt(np.diag(param_covariance_matrix))[1])
    a_err, mu_err, sig_err = np.sqrt(np.diag(param_covariance_matrix))

    new_px=np.linspace(OPTS.lgl[0],OPTS.lgh[0],100)
    

    plt.figure(0)
    plt.errorbar(T, (DCF+1.5), DCFERR, color='k', ls='-', capsize=0)
