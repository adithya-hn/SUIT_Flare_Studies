...
def process_one_frame(i, suit_files, hmi_fls, hmi_dt_array, ref_suit_map, ref_hmi_wcs):
    # --- SUIT map ---
    suit_map = Map(suit_files[i])
    base_time = Time(parse_time(suit_map.date))

    # --- closest HMI ---
    idx = np.argmin(np.abs(hmi_dt_array - base_time))
    hmi_map = Map(hmi_fls[idx])

    # --- CROTA2 handling ---
    if "CROTA2" in hmi_map.meta:
        map_rot_angl = int(hmi_map.meta.get("CROTA2"))
        if map_rot_angl > 5:
            hmi_map = hmi_map.rotate(angle=-180 * u.deg)
        else:
            hmi_map = hmi_map
    else:
        hmi_map = hmi_map
...
#-----------------Intial paths and params--------------------------
    ...
    hmi_imgs='/media/adithya/Adi_disk4/SUIT_flare_work/case8_nov01/data/HMI/HMI_cutouts/'
    ...
    if __name__ == "__main__":
        with Pool(processes=nproc) as pool:
            dt_array = list(
                tqdm(
                    pool.imap(worker, range(len(nb4_fls)), chunksize=2),
                    total=len(nb4_fls)
                )
            )
