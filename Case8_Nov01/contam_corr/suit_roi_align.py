...
if __name__=='__main__':
    ...
    f_seq = sorted(glob.glob(os.path.join(project_path,'data/raw/*NB01.fits')))
    seq = Map(f_seq, sequence=True)
    x1_initial=seq[0].meta["X1"]
    y1_initial=seq[0].meta["Y1"]
    for i in range(len(seq)):
        x1_val=seq[i].meta["X1"]
        y1_val=seq[i].meta["Y1"]
        #print(x1_val,y1_val)
