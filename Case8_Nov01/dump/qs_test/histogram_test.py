...
for fltr in Filters:
    ...
    search_fold=f'/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case2_June02/data/raw/' #Custom Folder
    pathlib.Path('histogram_test').mkdir(parents=True, exist_ok=True)
    ...
    for i in range(len(files)):
        ...
        def triple_gaussian(x, amp1, mean1, sigma1, amp2, mean2, sigma2, amp3, mean3, sigma3, amp4, mean4, sigma4, amp5, mean5, sigma5):
            gauss1 = amp1 * np.exp(-(x - mean1)**2 / (2 * sigma1**2))
            gauss2 = amp2 * np.exp(-(x - mean2)**2 / (2 * sigma2**2))
            gauss3 = amp3 * np.exp(-(x - mean3)**2 / (2 * sigma3**2))
            gauss4 = amp4 * np.exp(-(x - mean4)**2 / (2 * sigma4**2))
            gauss5 = amp5 * np.exp(-(x - mean5)**2 / (2 * sigma5**2))
        ...
        p0 = [
            ...
            np.max(n)/10, qs_guess+1000, 600, # Third Gaussian (brighter features)
            np.max(n)/15, qs_guess+1500, 600, # Third Gaussian (brighter features)
            np.max(n)/20, qs_guess+2000, 600 ] # Third Gaussian (brighter features)
        try:
            params, cov = curve_fit(triple_gaussian, bin_centers, n, p0=p0,maxfev=5000)

            amp1, mean1, sigma1, amp2, mean2, sigma2, amp3, mean3, sigma3, amp4, mean4, sigma4, amp5, mean5, sigma5 = params
            ...
            print(f"Bright Features (Gaussian 4): Mean = {mean4:.2f}, Sigma = {sigma4:.2f}")
            print(f"Bright Features (Gaussian 5): Mean = {mean5:.2f}, Sigma = {sigma5:.2f}")
            ...
            fit_vals = triple_gaussian(bin_centers, *params)
            ...
            ndof = len(bin_centers) - len(params)  # degrees of freedom
            ...
            ax1.bar(bin_centers, n, width=(bins[1]-bins[0]), color='gray', alpha=0.6, label='Histogram')
            x_fit = np.linspace(bin_centers.min(), bin_centers.max(), 2000)
            y_fit = triple_gaussian(x_fit, *params)
            ...
            y_gauss4 = amp4 * np.exp(-(x_fit - mean4)**2 / (2 * sigma4**2))
            y_gauss5 = amp5 * np.exp(-(x_fit - mean5)**2 / (2 * sigma5**2))

            ax1.plot(x_fit, y_gauss1, 'b--', label=f'Gaussian 1 (QS): μ={mean1:.2f}')
            ax1.plot(x_fit, y_gauss2, 'g--', label=f'Gaussian 2: μ={mean2:.2f}')
            ax1.plot(x_fit, y_gauss3, 'm--', label=f'Gaussian 3: μ={mean3:.2f}')
            ax1.plot(x_fit, y_gauss4, 'y--', label=f'Gaussian 4: μ={mean4:.2f}')
            ax1.plot(x_fit, y_gauss5, 'c--', label=f'Gaussian 5: μ={mean5:.2f}')

            # Bottom panel: residuals
            ax2.bar(bin_centers, residuals, width=(bins[1]-bins[0]), color='black', alpha=0.7)
            ax2.axhline(0, color='red', linestyle='--')
            ax2.set_xlabel("Pixel Intensity")
            ax2.set_ylabel("Residuals")
            ax2.set_title(f"Residuals (Reduced χ² = {reduced_chi2:.3f})")
            ax2.grid(True)
            #plt.figlegend(bbox_to_anchor=(0.001, 0.38, 0.4, 0.5))
            plt.tight_layout()

            ax1.set_ylabel("Counts")
            ax1.set_title("Triple Gaussian Fit to SUIT ROI Histogram")
            ax1.legend()
            ax1.grid(True)
            plt.legend()
            plt.grid(True)
