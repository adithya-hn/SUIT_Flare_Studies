...
#data1=(np.loadtxt(f'171_fd_lc.csv',delimiter=',',dtype='str')).transpose()
#data2=(np.loadtxt(f'171_roi_lc.csv',delimiter=',',dtype='str')).transpose()
data3=(np.loadtxt(f'NB03_c6_lc_data.csv',delimiter=',',dtype='str')).transpose()
data4=(np.loadtxt(f'NB08_c6_lc_data.csv',delimiter=',',dtype='str')).transpose()
data5=(np.loadtxt(f'MgII_2k_lc.csv',skiprows=1,delimiter=',',dtype='str')).transpose()

csv_filename = "goes_xray_lightcurve_20241101.csv"
...
#ax3.errorbar(dt1,count1,yerr=np.sqrt(count1)/area,fmt='tab:blue',capsize=2,markersize=1,linewidth=0.5,label='171')
#axs.errorbar(dt2,count2,yerr=np.sqrt(count2),capsize=2,fmt='tab:green',markersize=1,linewidth=0.5,label='171 roi')
ax2.errorbar(dt5,count5,yerr=np.sqrt(count5),capsize=2,fmt='tab:blue',markersize=1,linewidth=0.5,label='Mg II k FD')
#ax3.errorbar(dt4,count4,yerr=np.sqrt(count4),fmt='mo-',capsize=2,markersize=1,linewidth=0.5,label='Ca II h')
