...
data1=(np.loadtxt(f'NB03_c2_lc_data.csv',delimiter=',',skiprows=1,dtype='str')).transpose() #'NB03_Light_curve_data.dat'
data2=(np.loadtxt(f'NB08_c2_lc_data.csv',delimiter=',',skiprows=1,dtype='str')).transpose() 
data3=(np.loadtxt(f'NB04_c2_lc_data.csv',delimiter=',',skiprows=1,dtype='str')).transpose() 
Solexs=(np.loadtxt(f'fit_results_AL1_SOLEXS_20240602_SDD2_L1_2406020230_2406020530_TEMP_EM.txt',skiprows=1,dtype='str')).transpose()
...
#axs1_=axs[1].twinx()
axs[0].errorbar(time_array1[:159], (lc1_mean/qs1_mean)[:159],yerr=n_lc1_er[:159],fmt='tab:blue', marker="o",capsize=2,markersize=2,linewidth=0.5, label="SUIT Mg II k"); axs[0].legend(loc='lower right')
axs[0].errorbar(time_array1[159:], (lc1_mean/qs1_mean)[159:],yerr=n_lc1_er[159:],fmt='tab:blue', marker="o",capsize=2,markersize=2,linewidth=0.5, label="SUIT Mg II k"); axs[0].legend(loc='lower right')
axs[1].errorbar(time_array2[:80], (lc2_mean/qs2_mean)[:80],yerr=n_lc2_er[:80],fmt='black', marker="o",capsize=2,markersize=2,linewidth=0.5, label="SUIT Ca II H"); axs[1].legend(loc='lower right')
axs[1].errorbar(time_array2[80:91], (lc2_mean/qs2_mean)[80:91],yerr=n_lc2_er[80:91],fmt='black', marker="o",capsize=2,markersize=2,linewidth=0.5, label="SUIT Ca II H"); axs[1].legend(loc='lower right')
axs[1].errorbar(time_array2[91:], (lc2_mean/qs2_mean)[91:],yerr=n_lc2_er[91:],fmt='black', marker="o",capsize=2,markersize=2,linewidth=0.5, label="SUIT Ca II H"); axs[1].legend(loc='lower right')
...
axs[2].errorbar(helio_time_array, cdte,yerr=cdte_er,fmt='gray', marker="o",capsize=2,markersize=2,linewidth=0.5, label="HEL1OS (CdTe1+CdTe2)"); axs[2].legend(loc='lower right')
axs[3].errorbar(time_array4,sl_Em,yerr=sl_Em_er,fmt='gray', marker="o",capsize=2,markersize=2,linewidth=0.5, label='SoLEXS Emission Measure'); axs[3].legend(loc='lower right')
axs[4].errorbar(time_array4,sl_temp,yerr=sl_temp_er,fmt='g-', marker="o",capsize=2,markersize=2,linewidth=0.5, label='SoLEXS Temperature'); axs[4].legend(loc='lower right')
...
for i, ax in enumerate(axs):
    ax.text(0.02, 0.9, panel_labels[i],
            ...
            va='top', ha='left')
plt.savefig('case1_lc.png',dpi=300)
plt.close()
