...
for fltr in Filters:
    ...
    Plot_data_er=[]
    dates=[]
    box_pth=fol_nm+'/'+fltr+'/'+'/img1.2/1.2Box'
    pathlib.Path(fol_nm+'/'+fltr+'/img1.2').mkdir(parents=True, exist_ok=True)
    ...
    for i in range(len(Sequence)):
        ...
        F_name=f'{fol_nm}/{fltr}/img1.2/{fnm}.jpg'
        ...
        if Align_LC==True:
            ...
            F_name=f'{fol_nm}/{fltr}/img1.2/algn_{fnm}.jpg'
        ...
        coords = SkyCoord(Tx=(-555, -465) * u.arcsec, Ty=(-250, -160) * u.arcsec, frame=suit_map.coordinate_frame)
        ...
        er_coords = SkyCoord(Tx=(-400, -310) * u.arcsec, Ty=(-390, -480) * u.arcsec, frame=suit_map.coordinate_frame)
    ...
    np.savetxt(f'cbox_{fltr}_X1.4_Light_curve_data.csv',np.c_[date_array,fltr_count,fltr_count_err],delimiter=',',fmt='%s')
