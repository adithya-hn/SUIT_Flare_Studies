...
fol_nm='/Analysis/Research_Projects/Flare_studies/SUIT_Flares/June01_Flare/'#str(now.day)+'_'+str(now.month).zfill(2)+'_'+str(now.year).zfill(2)


Filters=['Temp']

search_fold='/Analysis/Research_Projects/Flare_studies/SUIT_Flares/June01_Flare/Flare_temp/Temp_Fits/'#'/scratch/suit_data/level1fits/2024/'+str(now.month).zfill(2)+'/'+str(now.day).zfill(2)+'/'+'normal_2k'+'/'
fdir =search_fold 
plot_data=[]
dates=[]
...
for fltr in Filters:
    ...
    for i in range(len(Sequence)):
        ...
        coords = SkyCoord(Tx=(-520, -390) * u.arcsec,Ty=(-323, -260) * u.arcsec,frame=suit_map.coordinate_frame,)
...
print(plot_data.shape)
np.savetxt(f'{fltr}_Light_curve_data.dat',plot_data)
np.savetxt(f'{fltr}_date_data.dat',dates,fmt='%s')
