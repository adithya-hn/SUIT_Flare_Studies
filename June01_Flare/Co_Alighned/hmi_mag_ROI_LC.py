...
for i in tqdm(range (len(Sequence))):
    ...
    #point1 = SkyCoord(-503*u.arcsec, -392*u.arcsec, frame=hmi_map.coordinate_frame) # suit_box
    ...
    #point2 = SkyCoord(-313*u.arcsec, -232*u.arcsec, frame=hmi_map.coordinate_frame) #suit box
    point2 = SkyCoord(-190*u.arcsec, -160*u.arcsec, frame=hmi_map.coordinate_frame) #full AR
    ...
    Thresh_data=np.where((suit_box.data > -900) & (suit_box.data < 900), 0, suit_box.data)#
...
np.savetxt(f'Th_fullbox_{param}_X1.4_Light_curve_data.csv',np.c_[date_array,fltr_count],delimiter=',',fmt='%s')
'''
plot_data=np.array(plot_data)
