...
#===============================
# Refference cutout considered to avoid the flare brightening
#===============================
...
fol_nm='/Analysis/Projects_Data/Flare_Data/Feb06_flare_data/Processed/'
...
search_fold='/Analysis/Projects_Data/Flare_Data/Feb06_flare_data/' #'/scratch/suit_data/level1fits/2024/'+str(now.month).zfill(2)+'/'+str(now.day).zfill(2)+'/'+'normal_2k'+'/'
fdir =search_fold 
for fltr in Filters:
    if fltr=='NB03':
        Ref_idx=10
    ...
    ref_img=sunpy.map.Map(files[Ref_idx])
    top_right = SkyCoord(484 * u.arcsec, 331 * u.arcsec, frame=ref_img.coordinate_frame)
    bottom_left = SkyCoord(384 * u.arcsec, -30 * u.arcsec, frame=ref_img .coordinate_frame)
