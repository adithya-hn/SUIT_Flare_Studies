...
Filters=['NB05','NB02']
search_fold=f'/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case9_Nov13/data/aligned_crop/' #Custom Folder
...
for fltr in Filters:
    ...
    for i in range(len(files)):
        ...
        rotation_angle=suit_map.meta.get("P_ANGLE")
        ...
        qs1.append(np.sum(er_box1.data))
        qs2.append(np.sum(er_box2.data))
        qs3.append(np.sum(er_box3.data))
        dates.append(suitMap.date.datetime)
        test_point.append(np.sum(test_box.data)) #test_box.meta.get("CMD_EXPT") test_box.meta.get("SHTR_STR"))#
        shut_angle.append(suit_map.meta.get("SHTR_STR"))
        exposure.append(suit_map.meta.get("CMD_EXPT"))
        meas_exp.append(suit_map.meta.get("MEAS_EXP"))
        ar_er.append (np.median(test_box.data))#np.sqrt((np.sum(test_box.data))*suit_map.meta.get("CMD_EXPT")/1000)/(test_box.data).size)
        qs1_er.append(np.median(er_box1.data))#np.sqrt(np.sum(er_box1.data*suit_map.meta.get("CMD_EXPT")/1000))/(er_box1.data).size)
        qs2_er.append(np.median(er_box1.data))#np.sqrt((np.sum(er_box2.data))*suit_map.meta.get("CMD_EXPT")/1000)/(er_box2.data).size)
        qs3_er.append(np.median(er_box1.data))#np.sqrt((np.sum(er_box3.data))*suit_map.meta.get("CMD_EXPT")/1000)/(er_box3.data).size)

    dates=np.array(dates)
    qs1=np.array(qs1)
    test_point=np.array(test_point)
    np.savetxt(f'{fltr}_ar_qs_th_count.csv',np.c_[dates,qs1,qs1_er,qs2,qs2_er,qs3,qs3_er,test_point,ar_er,shut_angle,exposure,meas_exp,tot_count,plot_data],comments='',header='Time,QS1_tot,qs1_md,QS2_tot,qs2_md,QS3_tot,qs3_md,AR_tot,AR_md,Shutter_Ang,CMD_Exp,Meas_Exp,Threshold_count,Thresh_area',delimiter=',',fmt='%s')
