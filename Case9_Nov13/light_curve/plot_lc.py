...
# Solexs=0#(np.loadtxt(f'csv_files/AL1_SOLEXS_20241113_SDD2_L1_puc_tb_fit_results_TEMP_EM.txt',skiprows=1,dtype='str')).transpose()
...
Solexs1=fits.open('csv_files/AL1_SOLEXS_20241112_SDD2_L1_puc_tb_fit_results_T_EM.fits')#[1].data
Solexs2=fits.open('csv_files/AL1_SOLEXS_20241113_SDD2_L1_puc_tb_fit_results_T_EM.fits')#[1].data
# solexs_fits1.info()
# print(solexs_fits1.data)
# print(Solexs1['TIME'])#,solexs_fits1[:1])
#-------------------------------------------------------
...
times = [datetime.datetime.strptime(d, "%Y-%m-%d %H:%M:%S.%f") if '.' in d else datetime.strptime(d, "%Y-%m-%dT%H:%M:%S") for d in dates_str]
...
sl_temp1 = np.array(Solexs1[1].data['temperature'])
sl_temp_er1= Solexs1[1].data['TEMPERATURE_ERR']
time1 = Solexs1[1].data['TIME']
emission_measure2 = Solexs1[1].data['em']

sl_temp2 = np.array(Solexs2[1].data['temperature'])
sl_temp_er2= Solexs2[1].data['TEMPERATURE_ERR']
time2 = Solexs2[1].data['TIME']
emission_measure2 = Solexs2[1].data['em']
time_array41=[]
time_array42=[]
for t in time1:
    time_array41.append(datetime.datetime.fromtimestamp(t)-datetime.timedelta(hours=5,minutes=30))
for t in time2:
    time_array42.append(datetime.datetime.fromtimestamp(t)-datetime.timedelta(hours=5,minutes=30))

time_array4=np.concatenate((time_array41,time_array42))

print(sl_temp1)
sl_temp=np.concatenate((sl_temp1,sl_temp2))
sl_temp_er=np.concatenate((sl_temp_er1,sl_temp_er2))
...
ll=np.power(10,5.4)
#axs1.set_ylim(ll,ul)
axs[0].axvspan(xmin=datetime.datetime(2024,11,12,22,28,30),xmax=datetime.datetime(2024,11,12,22,34,30),color='r',alpha=0.2)
axs1.set_ylabel('Excess intensity DN/s ', fontsize=20,color=scol[0])
axs[0].set_ylabel(r'Mg II h counts ($\times 10^8$ DN) ',color='k')
...
# Add global title
fig.suptitle(f"Flare Light Curves (SOL2024-11-13T00:22) ", fontsize=20, weight='bold')
