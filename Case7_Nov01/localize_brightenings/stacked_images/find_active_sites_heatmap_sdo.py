...
if base_image=='SDO':
    ...
    im2=ax.imshow(aia_map.data/1e3,cmap='gray',alpha=1,vmin=-2,vmax=2,transform=ax.get_transform(aia_map.wcs))
    #base_map.plot(axes=ax)
    #im2=ax.imshow(base_map.data/1e3,cmap='suit_nb04',norm=norm)
else:
    base_map.plot(axes=ax)
peak_map.draw_contours(axes=ax,levels=[np.max(peak_map.data)*.6],colors='magenta',lw=.5)
sns_cl = sns.color_palette("coolwarm",as_cmap=True)
...
im=ax.imshow(heat_masked/1000, origin='lower', cmap='rainbow', alpha=0.5)
