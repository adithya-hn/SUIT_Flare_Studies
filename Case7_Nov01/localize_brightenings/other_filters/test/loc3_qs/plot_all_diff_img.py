...
aia131_fl=pd.read_csv('aia131_region_lc.csv')
aia_dt= pd.to_datetime(aia131_fl['time'])
reg=aia131_fl['region_2']/aia131_fl['exposure']
...
for f in files:
    ...
    df.loc[dt > 120, param] = None  # gap threshold = 120 sec
    
    # Plot intensity with error bar (optional)
    #plt.errorbar(df['Time'], df['diff_count'], yerr=df['Diff_error'], alpha=0.6, label=f.split('/')[-1])
    ax.plot(df['Time'], df[param]/np.max(df[param]), label=f.split('/')[-1])   # simple line plot
...
plt.tight_layout()
plt.savefig(f'{param}_plot_all_diff_img_lc.png',dpi=300)
plt.show()
