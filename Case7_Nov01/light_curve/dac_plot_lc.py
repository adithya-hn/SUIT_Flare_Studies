...
# Add global title
fig.suptitle(f"Flare Light Curves (SOL2024-11-01T02:16) ", fontsize=20, weight='bold')
...
axs1=axs.twinx()
axs.errorbar(time_array1, (lc1_tot)/10e8,yerr=lc1_mean_er/10e8,fmt='ko',capsize=2,markersize=2,linewidth=0.5, label=r"SUIT Mg II h )")
axs1.errorbar(time_array5,nb3_counts,yerr=nb3_counts_er,color=scol[0], fmt='bo',capsize=2,markersize=2,linewidth=0.5,label='Excess intensity)')
axs1.set_yscale('log')
axs.set_ylabel(r'Mg II h counts ($\times 10^8$ DN/s) ',fontsize=18,color='k')
axs1.set_ylabel('Excess intensity DN/s ',fontsize=18,color=scol[0])
axs.set_xlabel(f"Time (UT)",fontsize=18) # Shared x-label
time_formatter = mdates.DateFormatter('%H:%M')  # Format as HH:MM
plt.gca().xaxis.set_major_formatter(time_formatter)
# axs.axvline(datetime.fromisoformat('2024-11-01T02:05:00.000'),color='orange',linestyle='--',label='GOES Flare start time')
# axs.axvline(datetime.fromisoformat('2024-11-01T02:16:00.000'),color='orange',linestyle='-',label='GOES Flare peak time')
axs.tick_params(axis='y', labelsize=16)
axs1.tick_params(axis='y', labelsize=16)
axs.tick_params(axis='x', labelsize=16)
for pk in suit_pks_dt:
    axs.axvline(pk,alpha=0.6,color='tab:purple')

lines, labels = axs.get_legend_handles_labels()
lines2, labels2 = axs1.get_legend_handles_labels()
axs.legend(lines + lines2, labels + labels2, loc='upper center',bbox_to_anchor=(0.4, 0.99))
plt.title(f'SUIT Mg II h light curve (2024-11-01)', fontsize=20, weight='bold')
plt.savefig(f'case{C_n}_mg2_nb3_lc_pks.png',dpi=300)
plt.close()
...
axs[0].errorbar(helio_time_array, cdte,yerr=cdte_er,color=scol[2], marker="o",capsize=2,markersize=2,linestyle='None', label="HEL1OS (10-30 keV)"); axs[0].legend(loc='upper center')
axs[1].errorbar(time_array4,sl_temp,yerr=sl_temp_er,color=scol[3], marker="o",capsize=2,markersize=2,linestyle='None', label='SoLEXS Temperature'); axs[1].legend(loc='upper center')
...
axs[-1].set_xlabel(f"Time (UT)",fontsize=18) # Shared x-label
axs[0].set_ylabel('HEL1OS counts/s',fontsize=18,color=scol[2])
axs[1].set_ylabel('Temperature (MK)',fontsize=18,color=scol[3])
fig.suptitle(f'HEL1OS and SoLEXS light curves (2024-11-01)', fontsize=20, weight='bold')
time_formatter = mdates.DateFormatter('%H:%M')  # Format as HH:MM
plt.gca().xaxis.set_major_formatter(time_formatter)
plt.savefig(f'case{C_n}_helio_solexs_lc.png',dpi=300)
plt.show()