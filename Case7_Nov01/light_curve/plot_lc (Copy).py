...
for idx in gap_indices:
    axs[0].errorbar(time_array1[start:idx+1], (lc1_tot)[start:idx+1]/10e8,yerr=lc1_mean_er[start:idx+1]/10e5, fmt="ko",capsize=2,markersize=2,linewidth=0.5)
    axs1.errorbar(time_array5[start:idx+1],nb3_counts[start:idx+1],yerr=nb3_counts_er[start:idx+1]*1e3,color=scol[0], fmt="o",capsize=2,markersize=2,linewidth=0.5)
    start = idx + 1
axs[0].errorbar(time_array1[start:], (lc1_tot)[start:]/10e8,yerr=lc1_mean_er[start:]/10e5, fmt="ko",capsize=2,markersize=2,linewidth=0.5, label=r"SUIT Mg II h (errors multiplied by $ 10^3$)")
axs1.errorbar(time_array5[start:],nb3_counts[start:],yerr=nb3_counts_er[start:]*1e3,color=scol[0], fmt="o",capsize=2,markersize=2,linewidth=0.5,label=r'Excess intensity (errors multiplied by $ 10^3$)')
axs[1].errorbar(helio_time_array, cdte,yerr=cdte_er,color=scol[2],capsize=2,markersize=2,linewidth=0.5,fmt='o', label="HEL1OS (10-30 keV)"); axs[1].legend(loc='upper center')
axs[2].errorbar(time_array4,sl_temp,yerr=sl_temp_er,color=scol[3], fmt="o",capsize=2,markersize=2,linewidth=0.5, label='SoLEXS Temperature'); axs[2].legend(loc='upper center')
axs2.plot(goes_dt,xrs_b/1e-6,color=scol[9],markersize=2,marker='o', linestyle='None', label='GOES: 1–8 Å')
axs1.set_yscale('log')
for pk in peaks_dt:
...
plt.savefig(f'dac_case{C_n}_lc.pdf',dpi=300)
plt.close()
