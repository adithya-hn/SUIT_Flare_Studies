...
nb3_counts=np.array(data1[3],dtype=float)
nb8_counts=np.array(data2[3],dtype=float)
...
ax2 = ax.twinx()
ax2.plot(time_array1[:nb3_c1],nb3_counts[:nb3_c1],'y')
ax2.plot(time_array1[nb3_c1:],nb3_counts[nb3_c1:],'y')
ax.errorbar(time_array1[:nb3_c1], (lc1_mean)[:nb3_c1],yerr=lc1_mean_er[:nb3_c1],fmt='tab:blue', marker="o",capsize=2,markersize=2,linewidth=0.5, label="SUIT Mg II h"); axs[0].legend(loc='lower right')
ax.errorbar(time_array1[nb3_c1:], (lc1_mean)[nb3_c1:],yerr=lc1_mean_er[nb3_c1:],fmt='tab:blue', marker="o",capsize=2,markersize=2,linewidth=0.5); axs[0].legend(loc='lower right')
ax.set_xlabel('Time')
ax.set_ylabel('Mg II h total counts', color='tab:blue')
ax2.set_ylabel('Threshold counts', color='black')
ax2.legend(['Brightenings'], loc='upper left')
ax.legend(['Mg II h counts'], loc='upper right')
plt.title(f'Light Curve for Mg II h ({date[:10]})')
time_formatter = mdates.DateFormatter('%H:%M')  # Format as HH:MM
plt.gca().xaxis.set_major_formatter(time_formatter)
plt.savefig(f'case{C_n}_nb3_lc.png',dpi=300)
plt.close()
...
ax2.plot(time_array1[:nb3_c1],nb3_counts[:nb3_c1],'y')
ax2.plot(time_array1[nb3_c1:],nb3_counts[nb3_c1:],'y')
ax.errorbar(helio_time_array[hel1:], cdte[hel1:],yerr=cdte_er[hel1:],fmt='tab:red', marker="o",capsize=2,markersize=2,linewidth=0.5, label="HEL1OS (CdTe1+CdTe2)"); axs[2].legend(loc='lower right')
#ax.errorbar(time_array1[:nb3_c1], (lc1_mean)[:nb3_c1],yerr=lc1_mean_er[:nb3_c1],fmt='tab:blue', marker="o",capsize=2,markersize=2,linewidth=0.5, label="SUIT Mg II h"); axs[0].legend(loc='lower right')
#ax.errorbar(time_array1[nb3_c1:], (lc1_mean)[nb3_c1:],yerr=lc1_mean_er[nb3_c1:],fmt='tab:blue', marker="o",capsize=2,markersize=2,linewidth=0.5); axs[0].legend(loc='lower right')
ax.set_xlabel('Time')
ax.set_ylabel('HEL1OS count rate (counts/min)', color='tab:blue')
ax.set_yscale('log')
ax2.set_ylabel('Threshold counts', color='black')
ax2.legend(['Brightenings'], loc='upper left')
ax.legend(['HEL1OS counts (10-20 keV)'], loc='upper right')
plt.title(f'Light Curve for Mg II h ({date[:10]})')
time_formatter = mdates.DateFormatter('%H:%M')  # Format as HH:MM
plt.gca().xaxis.set_major_formatter(time_formatter)
plt.savefig(f'case{C_n}_nb3_helios_lc.png',dpi=300)
plt.show()
