...
nb3_counts=np.array(spikes_nb3[3],dtype=float)/np.array(spikes_nb3[2],dtype=float)
...
fig, axs = plt.subplots(4, 1, sharex=True, figsize=(12,14), gridspec_kw={'hspace': 0})  # no vertical spacing 
...
for idx in gap_indices:
    axs[0].errorbar(time_array1[start:idx+1], (lc1_tot)[start:idx+1]/10e8,yerr=lc1_mean_er[start:idx+1]/10e8,fmt='k', marker="s",linestyle='--',capsize=2,markersize=3,linewidth=1  )
    ...
    start = idx + 1
axs[0].errorbar(time_array1[start:], (lc1_tot)[start:]/10e8,yerr=lc1_mean_er[start:]/10e8,fmt='k', marker="s",linestyle='--',capsize=2,markersize=3,linewidth=1, label=r"SUIT Mg II h (errors multiplied by $ 10^3$)")
...
ll=np.power(10,5.4)
#axs1.set_ylim(ll,ul)
axs[0].axvspan(xmin=datetime(2024,11,1,0,16),xmax=datetime(2024,11,1,0,23),color='r',alpha=0.2)
axs1.set_ylabel('Excess intensity (DN/s)', fontsize=20,color=scol[0])
axs[0].set_ylabel(r'Mg II h counts ($\times 10^9$ DN/s) ',color='k')
...
axs2.yaxis.set_label_coords(1.04, 0.6)
axs2.set_ylim(3,16)
...
plt.savefig(f'sigma_test_case{C_n}_lc.png',dpi=300)
plt.close()
