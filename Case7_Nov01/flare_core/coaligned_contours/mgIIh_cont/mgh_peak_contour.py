...
filter2_fold = '/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case7_Nov01/data/processed/aligned_fits/NB04/'

Tx1_qs1,Ty1_qs1=-540,350
Tx2_qs1,Ty2_qs1=-450,430

# Load the Filter 1 image and compute its contours
ref_mg_map = sunpy.map.Map('/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case7_Nov01/data/processed/aligned_fits/NB03/SUT_T24_1592_000636_Lev1.0_2024-11-01T02.16.26.605_0983NB03.fits')
mgh_map=sunpy.map.Map('/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case7_Nov01/data/processed/aligned_fits/NB04/SUT_C24_0413_000635_Lev1.0_2024-11-01T00.23.19.328_0973NB04.fits')
...
print('Threshold: ', Thresh_val,ref_mg_map_data.shape)
...
top_labels = [regions[0].label]#, regions[1].label, regions[2].label] #, regions[4].label]
...
plt.savefig('Mg_II_H_map.jpg')
plt.show()

# Process all Filter 2 images and overlay Filter 1 contours
filter2_files = glob.glob(filter2_fold + '*3NB04.fits')
...
for filter2_file in filter2_files:
    ...
    qs_coords_=SkyCoord(Tx=(Tx1_qs1,Tx2_qs1) * u.arcsec, Ty=(Ty1_qs1,Ty2_qs1) * u.arcsec,   frame=filter2_map.coordinate_frame)
    qs_submap = filter2_map.submap(qs_coords_)
    mg_qs_data = qs_submap.data * 1000 / qs_submap.meta.get('CMD_EXPT')
    filter2_data = filter2_map.data * 1000 / qs_submap.meta.get('CMD_EXPT')
    ...
    filter2_map.draw_quadrangle(qs_coords_,axes=ax,edgecolor="blue",linestyle="-",linewidth=1,label='QS region',alpha=0.5)
    #plt.colorbar()

    # Save the plot
    output_filename = os.path.join(output_dir, os.path.basename(filter2_file)[:-5] + '_overlay.jpg')
    #ax.set_colorbars()
    plt.title(f"Mg II h {filter2_map.date}", fontsize=16)
...
results_df.to_csv(('nb04_contours.csv'), index=False)