...
def stix_genspec(stix_file, tstart, tstop, outfile):
    ...
    tstart = Time(tstart)
    tstop  = Time(tstop)
    print('spec_start time:',time[0].datetime)
    ...
    exposure = data['EXPOSURE'][mask]  # seconds, exposure of each channel
    ...
    print('timedel',total_exposure,'exposure: ',exposure,'Rate len',len(rate))
