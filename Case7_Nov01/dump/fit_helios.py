...
if(gti_filename is not None):
    for n,i,j in zip(np.arange(len(gti_start_arr_UTC)), gti_start_arr_UTC, gti_stop_arr_UTC):
        plt.axvspan(i,j, color='green', alpha=0.5, label='_'*n + 'Good Time Intervals')
integration_interval = (datetime.datetime(2024,11,1,1,30), datetime.datetime(2024,11,1,1,37)) # not clear after this step
...
pi_filename  = '/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case7_Nov01/data/helios/hel1os_cdte_spectra_cdte1.fits'
gti_filename = '/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case7_Nov01/data/helios/gticdte1.fits'
cdte_arf_filename = '/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case7_Nov01/data/helios/hel1os_cdte_arf_v03.fits'
cdte_srf_filename = '/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case7_Nov01/data/helios/hel1os_cdte_srf_v03.fits'
lc_filename = '/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Case7_Nov01/data/helios/lightcurve_cdte1.fits' 
...
lcurve_idx = 1 # can vary between 1 to 5
plot_times_UTC = np.array([datetime.strptime(i[2:], '%y-%m-%dT%H:%M:%S.%f') for i in lc_hdul[lcurve_idx].data['ISOT']]).astype(datetime)
plot_lcurve = lc_hdul[lcurve_idx].data['CTR']

pi_hdul = fits.open(pi_filename)
pi_hdul.info()

integration_interval = (datetime(2024,11,1,1,32,30), datetime(2024,11,1,1,34,30))
bg_interval          = (datetime(2024,11,1,1,15), datetime(2024,11,1,1,20)) 
plot_interval        = (datetime(2024,11,1,1,00), datetime(2024,11,1,1,40))


idx = (plot_times_UTC>plot_interval[0]) & (plot_times_UTC<plot_interval[1])
plt.errorbar(x=plot_times_UTC[idx], y=plot_lcurve[idx], yerr=np.sqrt(plot_lcurve[idx]), fmt='.', capsize=5, c='b', label='Light curve');
if(gti_filename is not None):
    for n,i,j in zip(np.arange(len(gti_start_arr_UTC)), gti_start_arr_UTC, gti_stop_arr_UTC):
        plt.axvspan(i,j, color='green', alpha=0.5, label='_'*n + 'Good Time Intervals')
...
plt.axvspan(*integration_interval, color='r', alpha=0.5, label='Spectrum Integration Period')
plt.axvspan(*bg_interval, color='orange', alpha=0.5, label='Bg interval')
plt.legend(loc='upper right')
plt.grid(); plt.yscale('log')
plt.xlim(plot_times_UTC[idx][0], plot_times_UTC[idx][-1])
plt.xlabel('Time [UT]')
...
bg_idx= (pi_times_MJD>Time(bg_interval[0].isoformat(), format='isot').mjd) & (pi_times_MJD<Time(bg_interval[1].isoformat(), format='isot').mjd)
int_time = len(np.where(pi_idx)[0])*cadence

spec = np.sum(pi_hdul[1].data['COUNTS'][pi_idx,:], axis=0) #-np.sum(pi_hdul[1].data['COUNTS'][bg_idx,:], axis=0)
...
custom_spec.loglikelihood= 'cstat'

# select the energy range to fit
custom_spec.energy_fitting_range = [10,17]
custom_spec.params["T1_spectrum1"] = {"Status":"free", "Value":30, "Bounds":(1, 100)}
custom_spec.params["EM1_spectrum1"] = {"Status":"free", "Value":28100, "Bounds":(10, 100000)}

tol = 1e-20
minimiser_results = custom_spec.fit(tol=tol, options={"maxiter": 5000})
...
spec_mcmc = custom_spec.run_mcmc(number_of_walkers=4, steps_per_walker=1200,)
custom_spec.burn_mcmc = 250
...
T_fixed = custom_spec.params["T1_spectrum1"]['Value']    #7.22069 #
EM_fixed = custom_spec.params["EM1_spectrum1"]['Value']  #374.992 #

custom_spec.update_model = "(f_vth+thick_fn)"
custom_spec.energy_fitting_range = [10,35]
custom_spec.params["T1_spectrum1"] = {"Value":T_fixed, "Bounds":(1, 23)} # 26.06
custom_spec.params["EM1_spectrum1"] = {"Value":EM_fixed,"Bounds":(0.5, 300)} # 1.48
