...
fl_list = np.loadtxt('/Analysis/Research_Projects/Flare_studies/SUIT_Flares/Oct_22/aia_data/List_flare_frames.dat', dtype='str').transpose()
...
pathlib.Path("EM_fits").mkdir(parents=True, exist_ok=True)
pathlib.Path("EM_fits_").mkdir(parents=True, exist_ok=True)
...
for k in range(Length):
    try:
        ...
        npix=16 # spatiall binning for 1k image
        ...
        for j in range(0,30):
            ...
            er_Em[:,:,j]=np.sqrt(edem0[:,:,j]*(temps[j + 1] - temps[j]))
        ...
        hdu2=fits.PrimaryHDU(em0)
        hdu3=fits.PrimaryHDU(er_Em)
        hdu2.header=Header_part
        hdu3.header=Header_part
        hdu2.writeto('EM_fits/EM_{}.fits'.format(f_name),overwrite=True)
        hdu3.writeto('EM_fits_/EM_Er_{}.fits'.format(f_name),overwrite=True)
