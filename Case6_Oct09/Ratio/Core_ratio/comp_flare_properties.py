...
flare_light_curve_files = ['NB03_X1.8_core_lc_data.csv', 'NB04_X1.8_core_lc_data.csv', 'NB08_X1.8_core_lc_data.csv']

start_time_isot = '2024-10-09T01:25:00.000'
peak_time_isot = '2024-10-09T01:56:00.000'
end_time_isot = '2024-10-09T02:43:00.000'
noaa_region = 13848
goes_class = 'X1.8'
...
for file in flare_light_curve_files:
    ...
    props_text = (
                  ...
                  f"Box Area: {box_area}\n"
                  f"Pre flare level to QS ratio: {preflare_level/(box_area*quiet_sun_intensity)}")

    
    if file=='NB08_X1.8_core_lc_data.csv':
        plt.text(df['datetime'].min(), df['total_count'].max() * 0.93, props_text, fontsize=10,bbox=dict(facecolor='white', alpha=0.7))
    else:
        plt.text(df['datetime'].min(), df['total_count'].max() * 0.88, props_text, fontsize=10,
