...
fltr='NB02'
thresh_sig=3
suit_fls = glob.glob(suit_aligned_files + f'*{fltr}.fits')
print(len(suit_fls))
...
plt.show()
fig.savefig(f"{fltr}boxed_figure.png", dpi=300, bbox_inches='tight')
if not coords:
    raise RuntimeError("ROI selection cancelled or failed.")
...
np.savetxt(f"{fltr}boxes.csv", coords, fmt="%.6f",delimiter=',', header="xmin xmax ymin ymax")
