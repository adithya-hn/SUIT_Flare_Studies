...
int_roi = rgn_lc_data[:, 8].astype(float)/exposure #region 7
...
fig, axs = plt.subplots(4, 1, sharex=True, figsize=(12,16), gridspec_kw={'hspace': 0})
...
axs[1].errorbar(helio_time_array, cdte,yerr=cdte_er,color=scol[2], marker="o",capsize=2,markersize=2,linewidth=0.5, label="HEL1OS (10-30 keV)"); axs[1].legend(loc='upper center', frameon=True)
axs[2].errorbar(time_array4[:-9],sl_temp[:-9],yerr=sl_temp_er[:-9],color=scol[3], marker="o",capsize=2,markersize=2,linewidth=0.5, label='SoLEXS Temperature'); axs[2].legend(loc='upper center')
...
axs[0].legend(lines + lines2, labels + labels2, loc='upper center',bbox_to_anchor=(0.355, 0.99),frameon=True, )
...
ll=np.power(10,5.4)
#axs1.set_ylim(ll,ul)
axs[0].axvspan(xmin=datetime.datetime(2024,10,9,0,58,00),xmax=datetime.datetime(2024,10,9,1,3,40),color='r',alpha=0.2)
axs1.set_ylabel('Execss intensity DN/s ', fontsize=20,color=scol[0])
...
# Add global title
fig.suptitle(f"Flare Light Curves (SOL2024-10-09T01:56) ", fontsize=20, weight='bold')
