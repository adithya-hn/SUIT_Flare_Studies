...
def find_structured_peaks(flux, min_rise=3):
    peaks = []
    rise_count = 0
...
def find_peaks_with_one_dip(flux, min_rise=3):
    peaks = []
    n = len(flux)
    i = min_rise

    while i < n-1:
        # Check initial continuous rise
        rise_count = 0
        j = i
        
        while j > 0 and flux[j] > flux[j-1]:
            rise_count += 1
            j -= 1
        
        if rise_count >= min_rise:
            # Case 1: immediate drop → peak
            if flux[i] > flux[i+1]:
                peaks.append(i)
            
            # Case 2: one dip allowed
            elif flux[i] < flux[i+1]:
                # continue climbing → not peak yet
                pass
            
            elif flux[i] > flux[i+1] and i+2 < n and flux[i+2] > flux[i+1]:
                # one dip then rise → take next higher point
                k = i+1
                while k+1 < n and flux[k+1] > flux[k]:
                    k += 1
                peaks.append(k)
                i = k
...
flux_smooth1 = savgol_filter(flux, window_length=3, polyorder=2)
#sm=median_filter(flux, size=3)
peaks = find_peaks_min_rise(flux , 2)
#peaks = find_structured_peaks(sm, min_rise=2) 
# peaks = find_peaks_with_one_dip(flux, min_rise=3)
