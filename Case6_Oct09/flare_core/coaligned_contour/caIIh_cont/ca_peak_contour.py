...
filter2_fold = '/Analysis/Projects_Data/Flare_Data/Oct09_Flare_Data/Processed/Aligned_images/NB08/'
...
qs_coords =  SkyCoord(Tx=(-90, 10) * u.arcsec, Ty=(20, -80) * u.arcsec, frame=ref_mg_map.coordinate_frame)

#-------------
ca_map = sunpy.map.Map('/Analysis/Projects_Data/Flare_Data/Oct09_Flare_Data/Processed/Aligned_images/NB08/SUT_T24_1472_000592_Lev1.0_2024-10-09T01.48.24.118_0983NB08.fits') #not sure about this
ca_map_data = ca_map.data * 1000 / ca_map.meta.get('CMD_EXPT')
qs_coords1 =  SkyCoord(Tx=(-90, 10) * u.arcsec, Ty=(20, -80) * u.arcsec, frame=ca_map.coordinate_frame)
...
plt.imshow(ca_map_data, origin='lower', cmap='gray', vmin=0, vmax=6000)
...
for filter2_file in filter2_files:
    ...
    qs_coords3=SkyCoord(Tx=(-90, 10) * u.arcsec, Ty=(20, -80) * u.arcsec, frame=filter2_map.coordinate_frame)
    qs_submap = filter2_map.submap(qs_coords3)
    #qs_submap.peek()
    ...
    print('Counts:',qs_counts,qs_submap.meta.get('CMD_EXPT'))

    '''fig = plt.figure(figsize=(10, 10))
    ax = fig.add_subplot(projection=filter2_map)
    filter2_map.plot(axes=ax, cmap='gray')
    filter2_map.draw_quadrangle(qs_coords3, axes=ax, edgecolor="red", linestyle="-", linewidth=2, label='Region of interest')
    plt.show()'''
    
    # Calculate the counts in Filter 2 under the Filter 1 contours
    counts_under_contours = np.sum(np.where(msk==1, filter2_data, 0))/np.count_nonzero(msk)
